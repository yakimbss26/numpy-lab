...
(function () {
  ...
  Lab.register({
    ...
    blurb: '벡터의 크기·내적·사잇각부터 행렬곱이 한 칸씩 채워지는 과정까지, NumPy 가 다루는 수의 정체를 손으로 만져 봐요.',
    ...
    render: function (root) {
      ...
      root.appendChild(el('p', {
        class: 'lede',
        html: '이 장은 코드보다 <b>수학이 먼저</b>예요. 벡터·행렬을 한 번도 배우지 않았어도 괜찮아요 — ' +
          '이 앱의 모든 배열 연산은 결국 여기서 정의하는 몇 가지 개념(크기, 내적, 행렬곱)의 반복이에요. ' +
          '4장 이후에 나올 "축(axis)", "브로드캐스팅", "행렬곱" 은 전부 이 장에서 만든 그림 위에 쌓여요.'
      }));
      ...
      root.appendChild(el('p', {
        html: '<b>스칼라(scalar)</b>는 크기만 있는 양이에요 — 속력, 압력, 삼각형의 넓이처럼 숫자 하나로 끝나요. ' +
          '<b>벡터(vector)</b>는 크기에 <b>방향</b>까지 더해진 양이에요 — 속도, 힘, 가속도가 그래요. ' +
          '벡터는 굵은 소문자 <b>u, v, w</b> 로 쓰고, 시점 P 에서 종점 Q 로 가는 화살표로 그려요.'
      }));
      ...
      root.appendChild(UI.callout('why',
        '숫자 하나(스칼라)는 <code>shape ()</code>, 즉 축이 <b>0개</b>예요. ' +
        '벡터는 축이 <b>1개</b>인 배열이에요. "차원이 늘어나요"는 것은 결국 "축이 늘어나요"는 뜻이고, ' +
        '이 대응은 이 실습장 전체에서 한 번도 깨지지 않아요.'));
      ...
      (function () {
        ...
        function updateReadout() {
          ...
          readout.appendChild(UI.callout('why',
            '피타고라스 정리와 완전히 같은 식이에요. 직각삼각형의 빗변 길이를 구하듯, ' +
            '원점에서 종점까지의 화살표 길이를 성분의 제곱합의 제곱근으로 구해요.'));
          ...
          readout.appendChild(UI.callout('tip',
            state.k < 0
              ? '<b>k 가 음수</b>면 방향이 정반대로 뒤집혀요. 길이는 |k| 배가 돼요.'
              : (Math.abs(state.k) > 1
                ? '<b>|k| > 1</b> 이면 방향은 그대로, 길이만 <b>' + r2(Math.abs(state.k)) + '배</b> 늘어나요.'
                : '<b>|k| < 1</b> 이면 방향은 그대로, 길이만 줄어들어요.')));
          ...
          readout.appendChild(UI.callout('why',
            '내적의 결과에는 <b>shape 가 없어요</b> — 0차원, 즉 <b>스칼라</b> 하나예요. ' +
            '두 벡터를 "하나의 숫자"로 접어 버리는 연산이라고 생각하면 돼요. ' +
            '<code>np.dot</code>, <code>@</code>, <code>np.sum(u*v)</code> 는 이름만 다를 뿐 완전히 같은 계산이에요.'));
          if (Math.abs(dot1) < 1e-9) {
            readout.appendChild(UI.callout('tip', '<b>u·v = 0 → 두 벡터는 정확히 수직이에요.</b>', '수직!'));
          }

          readout.appendChild(el('h3', { class: 'h-sub', text: '사잇각 θ' }));
          if (normU < 1e-9 || normV < 1e-9) {
            readout.appendChild(UI.callout('trap', '길이가 0인 벡터와는 사잇각을 정의할 수 없어요.'));
          } else {
            ...
            readout.appendChild(UI.callout('why',
              dot1 > 1e-9 ? '내적이 <b>양수</b> → 사잇각이 90° 보다 작아요. 두 벡터가 대체로 같은 방향을 향해요.'
                : (dot1 < -1e-9 ? '내적이 <b>음수</b> → 사잇각이 90° 보다 커요. 두 벡터가 대체로 반대 방향을 향해요.'
        ...
        root.appendChild(UI.card({
          kicker: '시뮬레이터',
          title: '벡터 실험판 — u, v 를 손으로 끌어 보세요',
          note: '파란 화살표 <b>u</b> 와 주황 화살표 <b>v</b> 를 마우스로 끌면 아래 모든 값이 그 자리에서 다시 계산돼요. ' +
            '위 버튼으로 주 그림에 무엇을 겹쳐 그릴지 고르고, 아래 칩으로 딱 맞는 예를 불러와 보세요.',
          body: [
        ...
        root.appendChild(el('h3', { class: 'h-sub', text: '크기가 다른 벡터끼리는 애초에 더할 수 없어요' }));
        ...
        root.appendChild(UI.callout('why',
          '벡터의 합은 "같은 개수의 성분끼리" 라는 정의 자체가 전제예요. ' +
          '7장에서 배울 브로드캐스팅 규칙으로 봐도 마지막 축이 <b>2와 3으로 다르고 둘 다 1이 아니므로</b> 실패해요. ' +
          '수학의 정의와 NumPy 의 에러가 정확히 같은 이유에서 나와요.'));
      })();
      ...
      root.appendChild(el('p', {
        html: '2차원에서 본 거리 개념은 <b>n차원</b>으로 그대로 확장돼요. ' +
          '점 P(p₁, …, pₙ) 와 Q(q₁, …, qₙ) 사이의 거리는 각 성분 차이의 제곱을 모두 더해 제곱근을 씌운 것이에요.'
      }));
      root.appendChild(UI.ascii(
        'd(P, Q) = √( (p' + sub(1) + ' − q' + sub(1) + ')² + (p' + sub(2) + ' − q' + sub(2) + ')² + … + (p' + sub('n') + ' − q' + sub('n') + ')² )\n' +
        '        = √ Σᵢ (pᵢ − qᵢ)²\n' +
        '        = ‖P − Q‖                    <- 노름(norm)과 완전히 같은 식이에요'
      ));
      root.appendChild(UI.callout('tip',
        '이 거리 개념은 <b>K-평균(K-means)</b>, <b>K-최근접 이웃(K-NN)</b> 같은 분류·클러스터링 알고리즘의 재료예요. ' +
        '"가까워요" 를 숫자로 정의하는 방법이 바로 이 유클리드 거리이기 때문이에요.'));
      ...
      (function () {
        ...
        function updateInfo() {
          ...
          if (!state.query) {
            info.appendChild(el('p', { class: 'small', text: '평면을 클릭해 분류할 새 점을 놓아 보세요.' }));
            return;
          }
        ...
        root.appendChild(UI.card({
          ...
          note: '파란 점 5개는 부류 A, 주황 점 5개는 부류 B예요. 평면을 클릭해 새 점(노랑)을 놓으면 ' +
            '<b>모든 점까지의 유클리드 거리를 ND 로 계산</b>해 가장 가까운 k개를 찾고, 그 안에서 다수인 부류로 새 점을 분류해요.',
          body: [UI.controls([kSlider, resetBtn]), host, info, UI.fold('전체 점과의 거리 표 (거리순 정렬)', tableHost)]
        }));
        root.appendChild(UI.callout('why',
          '뉴런 하나의 판단(9쪽)도, K-NN 의 판단도 결국 "거리 계산 + 비교" 예요. ' +
          '벡터의 노름과 거리 개념 하나가 분류(classification)와 클러스터링(clustering)의 공통 재료가 돼요.'));
      })();
      ...
      root.appendChild(el('p', {
        html: '<b>행렬(matrix)</b>은 수를 직사각형으로 배열한 것이에요. m, n 이 양의 정수일 때 A 를 ' +
          '<b>m × n 행렬</b>(또는 (m, n) 행렬)이라 부르고, <b>행(row)은 가로, 열(column)은 세로</b>예요. ' +
          '이 방향을 거꾸로 외우는 것이 가장 흔한 실수예요.'
      }));

      (function () {
        var A = ND.arange(1, 13).reshape([3, 4]);
        var note = el('p', { class: 'small', style: { marginTop: '.6rem' }, text: '셀을 클릭해 보세요 — 수학 표기와 NumPy 표기가 어떻게 대응하는지 보여줘요.' });
        var g = UI.grid(A, {
        ...
        root.appendChild(UI.card({
          ...
          note: 'A 는 3 × 4 행렬이에요. <b>행은 가로줄, 열은 세로줄.</b> 위 머리글의 숫자는 NumPy 의 축 번호(0부터)예요.',
          body: [g, note]
        }));
        root.appendChild(UI.callout('trap',
          '수학은 <b>a₁₁ 부터 1로 세지만</b>, NumPy 는 <b>A[0, 0] 부터 0으로 세요.</b> ' +
          '그래서 수학의 aᵢⱼ 는 NumPy 의 <code>A[i−1, j−1]</code> 이에요. 이 어긋남을 잊으면 인덱스가 통째로 하나씩 밀려요.'));
      })();

      root.appendChild(el('h3', { class: 'h-sub', text: '행벡터와 열벡터 — 그리고 그 어느 쪽도 아닌 것' }));
      root.appendChild(el('p', {
        html: '행렬의 한 행(1 × n)을 <b>행벡터(row vector)</b>, 한 열(m × 1)을 <b>열벡터(column vector)</b>라 불러요. ' +
          '그런데 <code>np.array([1, 2, 3])</code> 은 shape 가 <code>(3,)</code> 이라 <b>행벡터도 열벡터도 아니에요.</b>'
      }));
      ...
      (function () {
        ...
        root.appendChild(UI.callout('why',
          '<code>(3,)</code> 은 축이 하나뿐이라 "가로/세로" 개념 자체가 없어요. ' +
          '진짜 행벡터·열벡터가 필요하면 <code>reshape</code> 로 축을 <b>2개</b>로 만들어야 해요. ' +
          '이 함정은 7장 브로드캐스팅에서 그대로 다시 나와요.'));
      })();
      ...
      root.appendChild(el('p', { html: '행렬의 <b>합</b>은 <b>같은 크기일 때만</b> 정의되고, 성분끼리 더해요.' } ));
      ...
      (function () {
        ...
        root.appendChild(el('h3', { class: 'h-sub', text: '크기가 다르면 애초에 더할 수 없어요' }));
        ...
        root.appendChild(UI.callout('why',
          '행렬의 합은 "자리가 맞는 성분끼리" 라는 정의부터 크기가 같아야 함을 요구해요. ' +
          '7장의 브로드캐스팅은 <b>특정 조건(한쪽 축이 1)에서만</b> 이 규칙을 완화해 주는 것이지, 아무 크기나 봐주는 것이 아니에요.'));
      })();

      root.appendChild(el('h3', { class: 'h-sub', text: '행렬의 곱 — cᵢⱼ = Σₖ aᵢₖ bₖⱼ' }));
      root.appendChild(el('p', {
        html: 'A 가 m × n, B 가 n × p 행렬이면 곱 AB = C 는 m × p 행렬이에요. ' +
          'C 의 (i, j) 성분은 <b>A 의 i번째 행 전체와 B 의 j번째 열 전체를 내적한 값</b>이에요 — 이 장 앞부분의 내적이 그대로 재사용돼요.'
      }));
      root.appendChild(UI.ascii(
        '      m   n         n   p           m   p\n' +
        '    ( A ) @ ( B )   =   ( C )\n' +
      ...
      (function () {
        ...
        function rebuild() {
          ...
          host.appendChild(UI.ascii(
            ...
            '          ^         ^\n' +
            (mismatch
              ? '          +----X----+  <- 안쪽 두 수가 달라요: ' + state.nA + ' ≠ ' + state.nB + '  → 계산 불가'
              : '          +---------+  <- 안쪽 두 수가 같아요: ' + state.nA + ' = ' + state.nB)
          ));
          ...
          if (mismatch) {
            ...
            host.appendChild(UI.callout('trap',
              '(m, n) @ (n, p) 규칙에서 <b>안쪽 두 수(n)가 같아야 해요.</b> ' +
              'A 의 열 개수와 B 의 행 개수가 다르면, A 의 행과 B 의 열을 짝지어 곱할 방법 자체가 없어요.'));
            return;
          }
          ...
          var detail = el('div', { class: 'small mono', style: { marginTop: '.5rem', minHeight: '1.4em' }, text: '결과 칸에 마우스를 올려 보세요.' });
          ...
          function resetC(cCells) {
            ...
            detail.textContent = '결과 칸에 마우스를 올려 보세요.';
          }
          ...
          var gC = UI.grid(C, {
            ...
            onHover: function (idx) {
              ...
              var flat = idx[0] * state.p + idx[1];
              if (flat >= revealed) { resetC(cC); detail.textContent = '아직 채워지지 않은 칸이에요 — "다음 칸 채우기" 를 눌러 보세요.'; return; }
          ...
          if (state.m === 2 && state.nA === 3 && state.nB === 3 && state.p === 2) {
            host.appendChild(UI.callout('tip',
              '지금 값은 수업 노트북 96번 셀 그대로예요: <code>a = np.arange(1,7).reshape(2,3)</code>, ' +
        ...
        root.appendChild(UI.card({
          kicker: '시뮬레이터',
          title: '행렬곱 단계별 시각화',
          note: '슬라이더로 A, B 의 크기를 바꿔 보세요. <b>n(A 의 열)과 n(B 의 행)을 다르게</b> 만들면 실제 에러가 나요. ' +
            '결과 칸에 마우스를 올리면 그 칸을 만든 A 의 행·B 의 열이 노랑으로 켜지고 계산식이 펼쳐져요.',
      ...
      (function () {
        ...
        root.appendChild(el('h3', { class: 'h-sub', text: 'A * B 는 행렬곱이 아니에요' }));
        root.appendChild(el('div', { class: 'stack-2' }, [
        ...
        root.appendChild(UI.statRow([
          ...
          { k: '값도 같은가', v: sameValue ? 'True' : 'False', sub: '같은 shape 라도 값은 달라요' }
        ]));
        root.appendChild(UI.callout('trap',
          '수학 시간의 "행렬의 곱"은 <code>@</code> 예요. <code>*</code> 는 성분별 곱(아다마르 곱)이며 ' +
          '<b>수학에서 배우는 행렬곱과는 전혀 다른 연산</b>이에요. NumPy 연산자 관점의 자세한 대비는 7장에서 더 다뤄요.'));

        root.appendChild(el('h3', { class: 'h-sub', text: '교환법칙이 성립하지 않아요 — AB ≠ BA' }));
        root.appendChild(el('div', { class: 'stack-2' }, [
          el('div', null, [UI.code('A @ B'), UI.out(ND.format(mm), { label: false })]),
          el('div', null, [UI.code('B @ A'), UI.out(ND.format(mmR), { label: false })])
        ]));
        root.appendChild(UI.callout('why',
          'A 의 행과 B 의 열을 짝짓는 순서(A @ B)와 B 의 행과 A 의 열을 짝짓는 순서(B @ A)는 서로 다른 계산이에요. ' +
          '숫자의 곱셈은 순서를 바꿔도 되지만(3×5 = 5×3), 행렬의 곱은 <b>일반적으로 순서를 바꿀 수 없어요.</b>'));
      })();
      ...
      (function () {
        ...
        root.appendChild(el('p', { html: '<b>전치(transpose)</b> A<sup>T</sup> 는 행과 열을 맞바꾼 행렬이에요. <code>.T</code> 와 <code>.transpose()</code> 는 같아요.' }));
        root.appendChild(el('div', { class: 'flow' }, [
        ...
        root.appendChild(UI.statRow([
          { k: 'np.shares_memory(A, A.T)', v: ND.sharesMemory(A, A.T) ? 'True' : 'False', sub: '값을 옮기지 않고 strides 순서만 뒤집은 뷰예요 — 4장에서 다시 나와요' }
        ]));
        ...
        root.appendChild(el('h3', { class: 'h-sub', text: '함정 — 1차원 배열의 .T 는 아무 일도 하지 않아요' }));
        var w = ND.arange(3);
        root.appendChild(UI.code('w = np.arange(3)\nw.shape       # ' + ND.shapeStr(w.shape) + '\nw.T.shape     # ' + ND.shapeStr(w.T.shape) + '   ← 그대로예요!'));
        root.appendChild(UI.callout('trap',
          '1차원 배열은 뒤집을 축이 <b>하나뿐</b>이라 <code>.T</code> 가 자기 자신을 그대로 돌려줘요. ' +
          '"행벡터를 열벡터로" 만들려면 2.3절에서 본 것처럼 <code>reshape(-1, 1)</code> 로 <b>축을 늘려야</b> 해요.'));
        ...
        root.appendChild(UI.callout('tip', '단위행렬 I 는 곱셈의 <b>1</b> 과 같은 역할이에요 — 어느 쪽에 곱해도 원래 행렬이 그대로 나와요.'));

        root.appendChild(el('h3', { class: 'h-sub', text: '연립방정식을 행렬로 — 역행렬과 solve' }));
        root.appendChild(el('p', { html: '다음 연립방정식을 행렬로 쓰면 <code>Ax = b</code> 예요.' }));
        root.appendChild(UI.ascii(
          '2x + 2y +  z = 9\n' +
          '2x −  y + 2z = 6\n' +
          ' x −  y + 2z = 5'
        ...
        root.appendChild(UI.statRow([
          ...
          { k: 'inv(A) @ b', v: '(' + r2(xInv.get([0])) + ', ' + r2(xInv.get([1])) + ', ' + r2(xInv.get([2])) + ')', sub: '같은 값이 나와요' }
        ]));
        root.appendChild(UI.callout('why',
          'det(A) ≠ 0 이므로 이 연립방정식은 <b>유일한 해</b>를 가져요 — x=1, y=2, z=3. ' +
          '<code>inv(A) @ b</code> 로도 같은 답을 얻지만, <code>np.linalg.solve</code> 는 역행렬을 실제로 만들지 않고 ' +
          '가우스 소거법으로 방정식을 직접 풀기 때문에 <b>반올림 오차가 덜 쌓이고 계산량도 적어요.</b> ' +
          '역행렬이 필요한 게 아니라 해만 필요하다면 <code>solve</code> 를 써요.'));
      })();
      ...
      root.appendChild(el('p', {
        html: '뉴런 하나는 입력 벡터 x 와 가중치 벡터 w 의 <b>내적</b>에 편향(bias) b 를 더해 값을 만들어요. ' +
          '뉴런이 여러 개(한 "층")면, 그 계산은 벡터의 내적이 아니라 <b>행렬곱</b>으로 한꺼번에 처리돼요. ' +
          '"인공지능 관련 지식에는 문과와 이과의 구별이 없어요" — 이 장의 벡터·행렬이 그 최소 공통분모예요.'
      }));
      ...
      (function () {
        ...
        function rebuild() {
          ...
          var detail = el('div', { class: 'small mono', style: { marginTop: '.4rem', minHeight: '1.4em' }, text: '뉴런 칸에 마우스를 올려 보세요.' });
          ...
          var gZ = UI.grid(Z, {
            ...
            onHover: function (idx) {
              if (!idx) {
                ...
                detail.textContent = '뉴런 칸에 마우스를 올려 보세요.'; return;
              }
              var j = idx[1];
          ...
          host.appendChild(UI.callout('tip',
            ...
            '(배치, 입력수) @ (입력수, 뉴런수) = (배치, 뉴런수). 입력이 3개, 뉴런이 2개인 층 하나를 한 번의 행렬곱으로 처리했어요.'));
        }
        rebuild();
        ...
        root.appendChild(UI.card({
          ...
          note: '슬라이더로 입력 x, 가중치 W, 편향 b 를 바꾸면 <code>x @ W + b</code> 가 실시간으로 다시 계산돼요. ' +
            '결과 z 의 칸에 마우스를 올리면 그 뉴런이 어떤 내적에서 나왔는지 노랑으로 보여줘요.',
          body: [
      ...
      root.appendChild(UI.quiz([
        {
          q: '4×4 행렬 <code>A</code> 에서 <code>A[1, 2]</code> 가 가리키는 것은 수학 표기로 무엇인가요?',
          choices: ['a₁₂ (1행 2열)', 'a₂₃ (2행 3열)', 'a₂₁ (2행 1열)', 'A 의 3번째 행 전체'],
          answer: 1,
          explain: 'NumPy 는 0부터 세므로 <code>A[1, 2]</code> 는 <b>2번째 행, 3번째 열</b>이에요. 수학 표기로는 aᵢⱼ 의 i, j 에 각각 1을 더한 <b>a₂₃</b> 와 같아요.'
        },
        {
          q: '(3, 4) 행렬과 (4, 2) 행렬을 <code>@</code> 로 곱한 결과의 shape 는?',
          choices: ['(3, 2)', '(4, 4)', '(3, 4)', '에러가 나요 — 크기가 안 맞아요'],
          answer: 0,
          explain: '(m, n) @ (n, p) → (m, p) 규칙 그대로예요. 안쪽 두 수 4 = 4 로 같으므로 계산되고, 바깥 두 수 3, 2 가 결과 shape 가 돼요.'
        },
        {
          q: '벡터 u, v 의 내적 <code>u · v</code> 의 결과는 무엇인가요?',
          choices: ['u, v 와 같은 shape 의 벡터', '축이 없는 스칼라(0차원) 값', '항상 양수인 값', 'u 또는 v 중 더 긴 쪽의 복사본'],
          answer: 1,
          explain: '내적은 두 벡터를 성분끼리 곱해 모두 더한 값 하나예요. <code>np.dot(u,v)</code>, <code>u @ v</code>, <code>np.sum(u*v)</code> 모두 같은 스칼라를 돌려줘요.'
        },
        {
          q: '<code>w = np.arange(3)</code> (shape (3,)) 에 대해 <code>w.T</code> 의 shape 는?',
          choices: ['(3, 1) — 열벡터가 돼요', '(1, 3) — 행벡터가 돼요', '(3,) 그대로예요 — 아무 일도 일어나지 않아요', '에러가 나요'],
          answer: 2,
          explain: '1차원 배열은 뒤집을 축이 하나뿐이라 <code>.T</code> 가 아무 효과를 내지 못해요. 열벡터가 필요하면 <code>w.reshape(-1, 1)</code> 처럼 축을 직접 늘려야 해요.'
        }
      ], { id: 'vector' }));
