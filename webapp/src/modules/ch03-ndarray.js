...
(function () {
  ...
  var DIMS = [
    {
      ...
      real: '오늘 낮 최고기온 <b>23.5 ℃</b> — 숫자 하나. 축이 하나도 없으니 인덱스를 붙일 자리가 없어요.',
    ...
    },
    {
      ...
      real: '<b>한 학생</b>의 과목별 점수 5개. 축이 하나 — 「몇 번째 과목」만 고르면 값이 나와요.',
      ...
      pic: function (a) {
        return panel('벡터 — 축 1개(axis 0)', 'a', [
          ...
          note('머리글 숫자가 axis 0 의 인덱스예요.')
        ]);
      }
    },
    {
      d: 2, kind: 'Matrix (2D tensor)', general: '(m, n)',
      real: '<b>반 성적표</b> — 학생 4명 × 과목 5개. 축이 둘이라 (학생, 과목) 두 개를 골라야 값 하나가 나와요. ' +
        '관절염 데이터(환자 × 날짜)도, 영화 평점표도 모두 2차원이에요.',
      code: 'scores = np.array([[88, 92, 75, 100, 67],\n' +
        '                   [70, 65, 80,  55, 90],\n' +
        '                   [95, 99, 91,  88, 100],\n' +
        '                   [60, 72, 68,  74, 71]])\n' +
      ...
      pic: function (a) {
        return panel('행렬 — 행(axis 0) × 열(axis 1)', 'a', [
          UI.grid(a, { axisLabels: true, highlight: function () { return 'a'; } }),
          note('세로 머리글이 axis 0(학생), 가로 머리글이 axis 1(과목)이에요.')
        ]);
      }
    },
    {
      d: 3, kind: '3D Tensor', general: '(a, b, c)',
      real: '<b>흑백 이미지 묶음</b> — (장수, 높이, 너비). mnist 손글씨가 대표적으로 (60000, 28, 28) 이에요. ' +
        '아래 그림은 (2, 3, 4) 짜리 축소판이고, 실제 이미지라면 칸의 숫자가 밝기(0~255)가 돼요.',
      ...
      pic: function (a) {
        return panel('3차원 — axis 0 을 층(層)으로 펼친 모습', 'a', [
          ...
          note('층 하나가 이미지 한 장이에요. 층 안은 (높이, 너비) 2차원이에요.')
        ]);
      }
    },
    {
      ...
      real: '<b>컬러 이미지 묶음</b> — (장수, 높이, 너비, 채널). 색은 빨강·초록·파랑 세 장을 겹쳐 만들므로 ' +
        '흑백보다 축이 하나 더 필요해요. 동영상은 여기에 시간축까지 붙어 (장면, 프레임, 높이, 너비, 채널) 5차원이 돼요.',
      ...
      pic: function (a) {
        ...
        box.appendChild(note('4차원은 한 장으로 못 그려요. 그래서 채널마다 2차원 판을 따로 떼어 나열했어요. ' +
          '같은 자리(같은 행·열)의 세 숫자가 한 픽셀의 색이에요.'));
        return box;
      }
    }
  ];
  ...
  function simDimExplorer() {
    ...
    return UI.card({
      kicker: '시뮬레이터',
      title: '차원 탐험기',
      note: '0차원부터 4차원까지 골라 봐요. shape 표기 · 실제 배열 그림 · 사례 · 만드는 코드가 함께 바뀌어요. ' +
        '<b>차원(ndim)이란 값 하나를 집으려면 인덱스를 몇 개 대야 하는가</b>이에요.',
  ...
  function secDims(root) {
    ...
    root.appendChild(p('NumPy 배열(ndarray)은 <b>같은 자료형의 숫자들을 격자에 담은 것</b>이에요. ' +
      '담긴 값이 무엇이든 배열의 성격은 <b>shape</b> 하나로 결정돼요. ' +
      '기온 한 개, 점수 다섯 개, 성적표, 사진 묶음이 모두 같은 자료구조로 표현돼요.'));
    ...
    root.appendChild(UI.callout('why',
      ...
      '<code>(1, 10, 100)</code> 으로 적혀 있어요. 이건 「샘플 1개를 예로 든 것」이라는 뜻이었는데, ' +
      '<b>1차원 배열의 shape 가 항상 (1,) 인 것으로 오해</b>하기 쉬워요. ' +
      '점수 5개를 담은 1차원 배열의 shape 는 <code>(5,)</code>이고, ' +
      '<code>(1,)</code>은 「원소가 딱 1개인 1차원 배열」이라는 전혀 다른 뜻이에요. ' +
      '그래서 이 실습장에서는 <code>(n,)</code>, <code>(m, n)</code> 처럼 <b>일반형</b>으로 적어요.',
      '수업자료의 shape 표기를 바로잡았어요'));

    root.appendChild(UI.callout('tip',
      '채널을 어디에 두는지는 도구마다 달라요. 케라스/텐서플로는 <b>채널을 뒤에</b> 두어 ' +
      '(장수, 높이, 너비, 채널)이고, 파이토치는 <b>채널을 앞에</b> 두어 (장수, 채널, 높이, 너비)예요. ' +
      '수업자료의 (samples, channels, width, height) 는 파이토치식 표기예요. ' +
      '<b>축의 개수가 아니라 축의 순서가 약속</b>이므로, 남의 코드를 가져올 때 shape 를 반드시 찍어 봐야 해요.',
      '축의 순서는 약속일 뿐이에요'));

    root.appendChild(simDimExplorer());

    /* ---- (3,) vs (3,1) vs (1,3) ---- */
    root.appendChild(h3('(3,) 과 (3, 1) 과 (1, 3) 은 서로 달라요'));
    root.appendChild(p('값 세 개를 담은 배열은 세 가지 모양으로 만들 수 있어요. 담긴 숫자는 똑같지만 ' +
      '<b>축의 개수와 역할이 다르므로 연산 결과가 완전히 달라져요.</b>'));
    ...
    root.appendChild(el('div', { class: 'stack-2', style: { marginTop: '1rem' } }, [
      panel('차이가 드러나는 순간', null, [
        ...
        note('shape 가 (3,) 과 (3,1) 로 다르면 브로드캐스팅이 일어나 ' +
          ND.shapeStr(sum.shape) + ' 표가 만들어져요. 자세한 규칙은 6장에서 다뤄요.')
      ]),
      panel('1차원 배열의 T 는 아무 일도 하지 않아요', null, [
        UI.code('v.shape              # ' + ND.shapeStr(v1.shape) +
          '\nv.T.shape            # ' + ND.shapeStr(v1.T.shape) + '  ← 그대로!\n' +
          'v.reshape(3,1).T.shape   # ' + ND.shapeStr(col.T.shape)),
        UI.callout('trap',
          '「전치하면 가로가 세로가 된다」고 외우면 1차원에서 반드시 틀려요. ' +
          '<b>축이 하나뿐이면 뒤집을 순서가 없어요.</b> 열벡터가 필요하면 ' +
          '<code>reshape(-1, 1)</code> 이나 <code>v[:, None]</code> 을 써야 해요.')
      ])
    ]));
  }
  ...
  function simColumnPick(sample) {
    var EXPRS = [
      { v: 'data[:, 0]', why: '정수 0 으로 열을 지정 → <b>열 축이 사라져</b> 1차원이 돼요.' },
      { v: 'data[:, :1]', why: '슬라이스 <code>:1</code> 로 열을 지정 → <b>열 축이 길이 1로 남아</b> 2차원이에요.' },
      { v: 'data[:, 2]', why: 'rating 열만 1차원으로 뽑았어요. 통계를 낼 때 보통 이 모양을 써요.' },
      { v: 'data[0, :]', why: '0번 행 하나 → 행 축이 사라져 1차원 (열 개수만 남아요).' },
      { v: 'data[0:1, :]', why: '0번 행 하나지만 슬라이스라서 행 축이 남아 2차원이에요.' }
    ];
    var state = { i: 0 };
    var host = el('div');
    ...
    return UI.card({
      kicker: '시뮬레이터',
      title: '열 하나 뽑기 — 정수와 슬라이스는 결과의 차원이 달라요',
      note: '수업 셀 5 는 <code>data[:, :1]</code> 을 써요. <code>data[:, 0]</code> 과 뽑히는 <b>숫자는 같은데 shape 가 달라요.</b> ' +
        '규칙은 하나예요 — <b>정수는 축을 없애고, 슬라이스는 축을 남겨요.</b>',
      body: [UI.controls([seg]), host]
    });
  }
  ...
  function secRatings(root) {
    ...
    root.appendChild(p('MovieLens 는 미네소타 대학 GroupLens 연구팀이 모아 공개하는 영화 평점 자료예요. ' +
      '이 실습에 쓰는 <code>ra.csv</code> 는 <b>사용자 610명이 영화 9,724편에 매긴 평점 100,836개</b>를 담고 있어요. ' +
      '한 줄이 「누가 · 어떤 영화에 · 몇 점을 · 언제」 매겼는지를 기록한 <b>2차원</b> 데이터예요. ' +
    ...
    if (!sample || !meta) {
      root.appendChild(UI.callout('trap',
        '이 빌드에는 영화 평점 데이터가 들어 있지 않아요. <code>수업자료/ra.csv</code> 를 놓고 ' +
        '<code>node webapp/build.js</code> 를 다시 실행하면 이 절의 숫자가 살아나요.'));
      return;
    }
    ...
    root.appendChild(UI.table(
      ...
      [
        { a: "'수업자료/ra.csv'", m: '읽을 파일. 첫 인자라 이름 없이 넘겨요.' },
        { a: "delimiter=','", m: '값을 나누는 문자. <b>기본값은 공백</b>이므로 CSV 는 반드시 콤마를 지정해야 해요.' },
        { a: 'skiprows=1', m: '맨 앞 1줄(헤더)을 버려요. ' + '<code>' + UI.esc(meta.header) + '</code> 이 그 줄이에요.' },
        { a: 'dtype (생략됨)', m: '기본값이 <code>float64</code>. 그래서 정수처럼 보이는 userId 도 <code>1.0</code> 으로 들어와요.' }
      ]
    ));
    root.appendChild(p('<code>skiprows=1</code> 을 빼면 어떻게 될까? 헤더 줄의 <code>userId</code> 라는 ' +
      '<b>글자를 실수로 바꿀 수 없으므로</b> 읽기 자체가 실패해요.'));
    ...
    root.appendChild(note('메시지 문구는 NumPy 버전에 따라 조금 다르지만 뜻은 같아요 — 글자를 숫자로 바꿀 수 없어요.'));
    ...
    root.appendChild(p('<code>type</code> 이 <code>numpy.ndarray</code> 라는 것이 중요해요. ' +
      '파이썬 <code>list</code> 가 아니라 <b>NumPy 배열</b>이라서 <code>.shape</code>, <code>.ndim</code> 같은 ' +
      '속성과 반복문 없는 계산을 쓸 수 있어요.'));

    root.appendChild(UI.callout('ver',
      'MovieLens 데이터는 <b>재배포가 금지</b>되어 있어요(라이선스 조건). 그래서 이 페이지에 실린 ' +
      num(meta.sampleRows) + '행은 실제 파일이 아니라 <b>구조만 같게 만든 합성 데이터</b>다 — ' +
      '열 구성(<code>userId, movieId, rating, timestamp</code>)과 값의 범위는 실제와 같지만 내용은 달라요. ' +
      '반면 전체 shape <code>' + ND.shapeStr(meta.trueShape) + '</code> 와 아래 집계값(사용자 수, 영화 수, 평점 평균, 분포)은 ' +
      '<b>실제 파일에서 계산한 사실</b>이에요. ' +
      '실제 데이터로 실습하고 싶다면 grouplens.org/datasets/movielens 에서 직접 받아 ' +
      '주피터에서 <code>np.loadtxt</code> 로 열어 보세요 — 그것이 이 절의 진짜 목표예요.',
      '이 페이지의 평점 표본은 합성 데이터예요'));
    ...
    root.appendChild(note('열 3 의 <code>' + ts0 + '</code> 은 1970년 1월 1일부터 흐른 <b>초</b>예요. ' +
      '날짜로 바꾸면 ' + new Date(ts0 * 1000).toISOString().slice(0, 10) + ' 이에요. ' +
      '값이 모두 float64 라 <code>' + ND.fmtScalar(ts0, 'float64') + '</code> 처럼 소수점이 붙어요.'));
    ...
    root.appendChild(note('이 다섯 값은 ' + num(meta.trueShape[0]) + '행 <b>전체</b>로 정확히 계산해 이 페이지에 새겨 넣은 값이에요.'));
    ...
    root.appendChild(note('막대는 <b>' + srcLabel + '</b>을 이 자리에서 세어 그린 것이에요. ' +
      '사람들은 3.0, 4.0 처럼 <b>정수 점수를 훨씬 많이 주고</b>, 0.5점 단위는 드물게 써요.'));
    ...
    root.appendChild(note('여기 나온 평균 ' + m.toFixed(6) + ' 은 <b>합성 표본</b>에서 나온 값이므로, ' +
      '실제 전체 평균 ' + meta.ratingMean + ' 과 같을 이유가 없어요. ' +
      '중요한 것은 숫자가 아니라 <b>계산하는 방법</b>이에요 — <code>data[:, 2]</code> 로 평점 열만 꺼내 ' +
      '<code>.mean()</code> 을 부른다는 절차는 실제 파일에서도 똑같아요.'));
  }
  ...
  function simPixelExplorer() {
    ...
    function rebuild() {
      ...
      host.appendChild(el('div', { class: 'stack-2' }, [
        panel('그림으로 — imgs[' + state.k + ']', null, [
          ...
          note('밝을수록 큰 값이에요. 진한 파랑 = 0(배경), 연한 파랑 = 255(글씨). 주황 테두리가 지금 고른 픽셀이에요.')
        ]),
        panel('숫자로 — 같은 배열, 칸을 눌러 봐요', 'a', [
          UI.grid(layer, {
    ...
    return UI.card({
      kicker: '시뮬레이터',
      title: '가짜 손글씨 픽셀 탐험기',
      note: '<b>솔직히 밝혀요 — 이 데이터는 진짜 mnist 가 아니에요.</b> 이 페이지는 인터넷에 연결하지 않고 ' +
        '혼자 도는 파일이라 60,000장을 담을 수 없어요. 그래서 28×28 대신 <b>8×8 로 줄인 숫자 모양 3장을 손으로 찍어</b> ' +
        '(장수, 높이, 너비) = ' + '(3, 8, 8) 3차원 배열로 쌓았어요. 구조는 mnist 와 완전히 같아요.',
  ...
  function secMnist(root) {
    ...
    root.appendChild(p('mnist 는 사람이 손으로 쓴 숫자 0~9 를 <b>28×28 흑백 이미지</b>로 모아 둔 자료예요. ' +
      '학습용 60,000장과 시험용 10,000장으로 나뉘어 있어, 학습 데이터의 shape 는 <code>(60000, 28, 28)</code> 이 돼요. ' +
      '머신러닝 입문에서 가장 많이 쓰이는 3차원 데이터예요.'));
    root.appendChild(p('<b>케라스(Keras)</b>는 딥러닝 모델을 간단하게 만들고 훈련시키는 프레임워커요. ' +
      '쓰기 쉬운 고수준 API 를 제공하고, 실제 계산은 백엔드 엔진인 텐서플로가 맡아요. ' +
      'mnist 는 케라스에 예제 데이터로 딸려 있어 한 줄로 불러올 수 있어요.'));
    ...
    root.appendChild(UI.callout('tip',
      '이 절은 <b>NumPy 를 배우는 데 꼭 필요한 부분이 아니에요.</b> keras 와 tensorflow 를 설치해야 하고, ' +
      '설치는 느리고 자주 실패해요. 목적은 딱 하나 — <b>3차원 배열이 실제로 어디에 쓰이는지 눈으로 보는 것</b>이에요. ' +
      '그 목적은 아래 시뮬레이터로도 충분히 달성돼요.'));

    var px = 60000 * 28 * 28;
    var one = ND.dtypeInfo('uint8').itemsize, eight = ND.dtypeInfo('float64').itemsize;
    root.appendChild(p('숫자로 감을 잡아 봐요. 학습 이미지의 원소 개수는 ' +
      '<code>60000 × 28 × 28 = ' + num(px) + '</code> 개예요. ' +
      'dtype 이 <code>uint8</code>(1바이트)이니 <code>nbytes</code> 는 ' + num(px * one) + ' 바이트, 약 ' +
      (px * one / 1024 / 1024).toFixed(1) + ' MiB 예요. ' +
      '만약 이걸 <code>float64</code>(8바이트)로 바꿔 담으면 ' + num(px * eight) + ' 바이트 = 약 ' +
      (px * eight / 1024 / 1024 / 1024).toFixed(2) + ' GiB 로 <b>8배</b>가 돼요. ' +
      '<b>dtype 선택이 곧 메모리 선택</b>이라는 것을 여기서 알 수 있어요.'));

    root.appendChild(UI.callout('trap',
      '<code>test_labels[6]</code> 이 몇인지는 <b>여기서 단정하지 않아요.</b> ' +
      '이 실습장 안에는 실제 mnist 파일이 없으므로 확인할 방법이 없고, ' +
      '확인하지 않은 값을 적어 두면 그것부터 외우게 돼요. ' +
      '코랩에서 직접 불러와 <code>plt.imshow</code> 로 그린 그림과 라벨이 맞는지 눈으로 확인해 봐요.',
      '확인할 수 없는 값은 적지 않아요'));

    root.appendChild(simPixelExplorer());

    root.appendChild(UI.callout('why',
      '이미지가 왜 3차원인가요? 흑백 이미지 한 장은 (높이, 너비) 2차원이에요. ' +
      '거기에 <b>「몇 번째 장인가」라는 축이 앞에 하나 붙어</b> 3차원이 돼요. ' +
      'NumPy 에서 <b>맨 앞 축(axis 0)은 관례적으로 「샘플 번호」</b>예요. ' +
      '그래서 <code>imgs[7]</code> 처럼 정수 하나만 주면 이미지 한 장이 통째로 나와요.'));
  }
  ...
  function trapOverflow() {
    ...
    function rebuild() {
      ...
      outHost.appendChild(UI.statRow([
        ...
        { k: '실제로 저장된 값', v: num(stored), sub: wrapped ? '접혔어요!' : '그대로 들어갔어요' }
      ]));
      ...
      if (wrapped) {
        outHost.appendChild(UI.steps([
          bits + '비트로는 서로 다른 값 <b>' + num(span) + '가지</b>밖에 구별할 수 없어요.',
          num(state.v) + ' 를 ' + num(span) + ' 로 나눈 나머지: <b>' +
            num(((Math.trunc(state.v) % span) + span) % span) + '</b>',
          info.kind === 'u'
            ? '<code>' + state.dt + '</code> 은 부호가 없으니 나머지가 그대로 저장값 <b>' + num(stored) + '</b> 이 돼요.'
            : '나머지가 ' + num(span / 2) + ' 이상이면 음수 쪽으로 넘어가요 → ' +
        ...
        outHost.appendChild(note('아직 범위 안이에요. 슬라이더를 더 밀어 ' + num(info.max) + ' 를 넘겨 봐요.'));
      }

      // 연산 중에도 넘친다
      var i8 = ND.array([100, 100], 'int8');
      outHost.appendChild(el('div', { style: { marginTop: '1rem' } }, [
        el('div', { class: 'panel-t', text: '덧셈만으로도 넘쳐요' }),
        ...
        note('100 + 100 = 200 은 int8 의 127 을 넘어요. NumPy 는 <b>경고만 띄우고 계산을 멈추지 않아요.</b> ' +
          '경고를 못 보고 지나가면 −56 을 정답으로 믿게 돼요.')
      ]));
    }
    ...
    rebuild();
    return el('div', null, [
      p('정수 dtype 은 <b>쓸 수 있는 비트 수가 정해져 있어요.</b> 범위를 넘는 값을 넣으면 시계처럼 한 바퀴 돌아 ' +
        '엉뚱한 값이 돼요. 이걸 <b>오버플로(overflow)</b> 또는 랩어라운드라고 해요.'),
      ctlHost, outHost,
      UI.callout('ver',
        'NumPy 2.0 부터 <b>파이썬 정수를 범위 밖 dtype 에 직접 넣으면 예외</b>가 나요: ' +
        '<code>np.array(300, dtype=np.int8)</code> → <code>OverflowError</code>. ' +
        '1.x 에서는 조용히 44 가 되었어요. 하지만 <code>astype</code> 이나 배열끼리의 연산은 ' +
        '<b>2.x 에서도 여전히 조용히 접혀요.</b> 즉 위험이 사라진 게 아니라 한 곳만 막힌 것이에요.')
    ]);
  }
  ...
  function trapAstype() {
    ...
    function rebuild() {
      ...
      if (v < 0) {
        outHost.appendChild(note('음수에서 차이가 확 드러나요. ' + v.toFixed(1) +
          ' 의 버림은 ' + cut.get([0]) + ' 이에요 — <b>내림(floor)이 아니라 0 쪽으로 자르기</b>예요. ' +
          'floor 는 ' + Math.floor(v) + ' 가 돼요.'));
      }
    }
    ...
    var rows = cases.map(function (v) {
      ...
      return {
        ...
        same: np === school ? '같아요' : '달라요!'
      };
    });

    rebuild();
    return el('div', null, [
      p('<code>astype(int)</code> 은 <b>반올림이 아니라 버림</b>이에요. 1.7 은 1 이 되고 −1.7 은 −1 이 돼요. ' +
        '반올림이 필요하면 <code>np.round</code> 를 써야 하는데, 이쪽도 학교에서 배운 반올림과 달라요.'),
      UI.controls([sl]), outHost,
      h3('np.round 는 「.5 는 짝수 쪽으로」 반올림해요'),
      UI.table(
      ...
      UI.callout('trap',
        ...
        '<b>3.0 이 아니라 2.0</b> 이에요. 이것을 <b>은행가 반올림</b>(round-half-to-even)이라고 해요. ' +
        '.5 를 항상 위로 올리면 많은 값을 더할 때 합이 조금씩 커지는 <b>편향</b>이 생기는데, ' +
        '짝수 쪽으로 번갈아 보내면 그 편향이 상쇄돼요. ' +
        '파이썬 내장 <code>round()</code> 도 같은 규칙이에요 — 이건 NumPy 의 변덕이 아니라 IEEE 754 표준이에요.')
    ]);
  }
  ...
  function trapFloat() {
    ...
    function rebuild() {
      ...
      outHost.appendChild(UI.statRow([
        ...
        { k: '차이', v: diff === 0 ? '0' : diff.toExponential(4), sub: diff === 0 ? '정확히 같아요' : '0 이 아니에요' },
      ...
      if (left !== c.t) {
        outHost.appendChild(UI.callout('why',
          ...
          '<b>' + String(left) + '</b> 을 갖고 있어요. 차이는 ' + diff.toExponential(4) + ' — ' +
          '눈에 보이지 않을 만큼 작지만 <b>0 이 아니므로 <code>==</code> 는 False</b> 예요.'));
      }
    }
    ...
    rebuild();
    return el('div', null, [
      p('컴퓨터는 실수를 <b>2진 소수</b>로 저장해요. 10진수 0.1 은 2진수로 쓰면 ' +
        '0.0001100110011… 처럼 <b>끝나지 않는 소수</b>라서, float64 는 유효숫자 약 15~17자리에서 잘라 저장해요. ' +
        '그래서 아주 작은 오차가 항상 남아요. 아래 계산은 이 브라우저가 지금 한 것이에요 — ' +
        'NumPy 도 같은 IEEE 754 규격을 쓰므로 결과가 같아요.'),
      UI.controls([sel]), outHost,
      h3('0.1 을 열 번 더하면 1.0 이 아니에요'),
      UI.code('a = np.full(10, 0.1)\na.sum()          # ?\na.sum() == 1.0\nnp.isclose(a.sum(), 1.0)'),
      UI.out(String(tenSum) + '\n' +
        ((tenSum === 1) ? 'True' : 'False') + '\n' +
        (ND.isclose(tenSum, 1) ? 'True' : 'False')),
      note('누적합을 보면 오차가 어디서 생기는지 보여요: ' +
        cum.flatValues().slice(0, 4).map(String).join(' → ') + ' → …'),
      UI.callout('tip',
        '실수를 <code>==</code> 로 비교하지 마세요. <code>np.isclose(a, b)</code> 로 값 하나를, ' +
        '<code>np.allclose(A, B)</code> 로 배열 전체를 비교해요. ' +
        '두 함수는 <b>상대오차 rtol=1e-05, 절대오차 atol=1e-08</b> 안에 들면 같다고 봐요. ' +
        '물리 실험값을 다루거나 수치해석 코드를 검증할 때 반드시 필요해요.'),
      UI.callout('why',
        '왜 float64 는 <b>정확한</b> 값을 안 쓰나요? 유리수를 정확히 담으려면 분모·분자를 따로 저장해야 하고 ' +
        '연산할 때마다 자리수가 불어나요. 과학 계산은 <b>고정된 8바이트로 일정한 속도</b>를 내는 것이 더 중요해요. ' +
        '정확한 10진 계산이 꼭 필요하면 파이썬 <code>decimal</code>, <code>fractions</code> 모듈을 써요.')
    ]);
  }
  ...
  function simDtypeLab() {
    ...
    return UI.card({
      ...
      note: 'dtype 은 「그냥 자료형」이 아니에요. <b>값을 조용히 바꿔 버리는 규칙</b>이에요. ' +
        '세 함정을 직접 건드려 봐요. 여기서 놓치면 나중에 계산 결과가 왜 틀렸는지 절대 못 찾아요.',
      body: [UI.controls([seg]), host]
    });
  }
  ...
  function secDtype(root) {
    ...
    root.appendChild(p('배열은 <code>np.array(리스트)</code> 로 만들어요. 여기서 list 와 결정적으로 갈려요 — ' +
      '<b>NumPy 배열은 한 가지 dtype 만 담아요.</b> 정수와 문자열을 섞어 넣을 수 없어요. ' +
      '이 제약 덕분에 값들을 메모리에 빈틈없이 나란히 놓을 수 있고, ' +
      'C 로 짜인 계산 루틴이 반복문 없이 한꺼번에 처리할 수 있어요. ' +
      '<b>NumPy 의 속도는 이 제약을 받아들인 대가로 얻은 것이에요.</b>'));
    ...
    root.appendChild(p('두 번째 인자 <code>float</code> 가 dtype 지정이에요. ' +
      '<b>문자열 <code>\'9\'</code> 가 숫자 9.0 으로 바뀌었고</b>, 원래 정수였던 1, 2, 3, 4 도 모두 실수가 되었어요. ' +
      '한 배열에 두 dtype 이 있을 수 없으니 <b>전부 하나로 맞춘 것</b>이에요.'));
    root.appendChild(UI.out(
    ...
    root.appendChild(el('div', { class: 'stack-2' }, [
      panel('dtype 을 빼면 — 전부 문자열이 돼요', 'b', [
        ...
        note('숫자 하나가 문자열이면 <b>NumPy 는 숫자를 문자열로 끌어올려요</b>(그 반대가 아니에요). ' +
          '<code>&lt;U21</code> 은 「유니코드 문자 최대 21자」라는 뜻이에요. 21 은 int64 를 글자로 적을 때 필요한 최대 자리수예요. ' +
          '이 실습장의 엔진에는 문자열 dtype 이 없어서 이 출력만은 실제 NumPy 결과를 옮겨 적었어요.')
      ]),
      panel('숫자가 아닌 글자는 실수로 못 바꿔요', 'b', [
        UI.code("np.array([1, 2, 'abc'], float)"),
        shown(function () { return UI.grid(ND.array(pyFloatList([1, 2, 'abc']), 'float64')); }),
        note("<code>'9'</code> 는 숫자로 읽히지만 <code>'abc'</code> 는 읽히지 않아요. " +
          '<b>변환 규칙이 있는 것과 없는 것의 차이</b>예요.')
      ])
    ]));

    root.appendChild(h3('나. 들쭉날쭉한 리스트는 배열이 될 수 없어요'));
    root.appendChild(p('NumPy 배열은 <b>빈틈 없는 직사각형 격자</b>예요. 행마다 길이가 다르면 격자가 되지 않아요.'));
    root.appendChild(el('div', { class: 'stack-2' }, [
      ...
      panel('길이가 다르면 오류', 'b', [
        ...
        note('둘째 행에 칸이 하나 비는데, 배열에는 「빈 칸」이 없어요.')
      ])
    ]));
    ...
    root.appendChild(UI.table(
      ...
      [
        ...
        { d: 'int64', b: ND.dtypeInfo('int64').itemsize, r: '−2<sup>63</sup> ~ 2<sup>63</sup>−1 (약 ±9.2×10<sup>18</sup>)', u: '<b>기본 정수형.</b> 웬만한 정수는 여기 다 들어가요' },
        { d: 'uint8', b: ND.dtypeInfo('uint8').itemsize, r: rng('uint8'), u: '<b>이미지 픽셀(0~255).</b> mnist 가 이 dtype 이에요' },
        { d: 'float32', b: ND.dtypeInfo('float32').itemsize, r: '유효숫자 약 7자리', u: 'GPU 딥러닝. 메모리·속도가 두 배 유리해요' },
        { d: 'float64', b: ND.dtypeInfo('float64').itemsize, r: '유효숫자 약 15~17자리', u: '<b>기본 실수형.</b> np.loadtxt 도 이걸로 읽어요' },
        { d: 'bool', b: ND.dtypeInfo('bool').itemsize, r: 'True / False', u: '조건 마스크(7장). 1바이트를 써요' },
        { d: '&lt;U21 등', b: '4×문자수', r: '유니코드 문자열', u: '계산 불가. 숫자 데이터에 섞이면 사고의 원인' },
        { d: 'object', b: 8, r: '파이썬 객체 아무거나', u: '최후의 수단. 사실상 list 라서 NumPy 의 장점이 사라져요' }
      ]
    ));

    root.appendChild(UI.callout('ver',
      '수업자료(2024년 3월, NumPy 1.x 기준)에는 <b>NumPy 2.0 에서 삭제된 이름들</b>이 있어요. ' +
      ...
      'Use np.nan instead.</code> 가 나요. 다행히 메시지가 고칠 방법까지 알려 줘요.'));
  }
  ...
  function simAttrCalc() {
    ...
    function rebuild() {
      ...
      outHost.appendChild(el('p', {
        ...
        html: ok
          ? '검증 통과 — <code>nbytes(' + num(a.nbytes) + ') = size(' + num(a.size) +
            ') × itemsize(' + item + ')</code> 가 엔진에서 실제로 성립해요.'
          : '검증 실패 — 엔진 버그예요.'
      }));
      ...
      if (size <= 48) {
        ...
        if (a.ndim === 2) {
          outHost.appendChild(panel('a.T — 축 순서를 뒤집은 뷰 ' + ND.shapeStr(a.T.shape), 'r', [
            ...
            note('값을 복사하지 않고 <b>같은 메모리를 다른 순서로 읽을 뿐</b>이에요. 자세한 것은 4·5장에서 다뤄요.')
          ]));
        }
      } else {
        outHost.appendChild(note('원소가 ' + size + '개라 격자는 생략했어요.'));
      }
      ...
      if (ND.isIntDtype(state.dt) && size - 1 > ND.DTYPES[state.dt].max) {
        outHost.appendChild(UI.callout('trap',
          ...
          '<code>np.arange</code> 가 만든 값들이 <b>중간에 접혔어요.</b> 격자를 보세요. ' +
          'shape·size·nbytes 는 영향을 받지 않아요 — 하지만 값은 이미 망가졌어요.'));
      }
    }
    ...
    return UI.card({
      ...
      note: '축 개수와 각 축의 길이, dtype 을 바꿔 봐요. ' +
  ...
  function secAttrs(root) {
    ...
    root.appendChild(p('배열을 받으면 <b>가장 먼저 속성을 찍어 봐요.</b> ' +
      '이 일곱 개면 배열의 정체가 거의 다 드러나요. 아래 값들은 ' +
      '<code>np.arange(30).reshape(5, 6)</code> 을 엔진이 실제로 만들어 읽은 것이에요.'));

    var a3 = ND.arange(30).reshape([5, 6]);
    root.appendChild(UI.table(
      [{ k: 'p', label: '속성' }, { k: 'm', label: '뜻', raw: true }, { k: 'v', label: '이 배열에서', num: true }],
      [
        { p: 'a.dtype', m: '원소 하나의 자료형. <b>배열 전체가 같은 dtype</b>이에요', v: a3.dtype },
        ...
        { p: 'a.T', m: '축 순서를 뒤집은 <b>뷰</b>(전치). 값을 복사하지 않아요', v: ND.shapeStr(a3.T.shape) }
      ]
    ));
    root.appendChild(note('shape 는 리스트가 아니라 <b>튜플</b>이라 <code>a.shape[0] = 3</code> 처럼 ' +
      '고칠 수 없어요. 모양을 바꾸려면 <code>reshape</code> 를 써야 해요(4장).'));
    ...
    root.appendChild(note('<code>4.5</code> 하나 때문에 1, 2, 3, 5, 6 도 모두 실수가 되었어요. ' +
      'size ' + arr1.size + ' × itemsize ' + arr1.itemsize + ' = nbytes ' + arr1.nbytes + ' 로 맞아떨어져요.'));

    root.appendChild(simAttrCalc());

    root.appendChild(UI.callout('ver',
      '<b>NumPy 2.0 부터 Windows 에서도 기본 정수형이 <code>int64</code></b> 예요. ' +
      '1.x 시절 Windows 에서는 <code>np.arange(30).dtype</code> 이 <code>int32</code> 였기 때문에, ' +
      '같은 코드가 Windows 에서 nbytes 120, 리눅스·맥에서 240 으로 <b>다르게 나왔어요.</b> ' +
      '옛 자료나 블로그에서 nbytes 값이 이 실습장과 다르면 이것이 원인일 수 있어요.'));
  }
    ...
    root.appendChild(UI.quiz([
      {
        q: '<code>data</code> 의 shape 가 <code>(100836, 4)</code> 일 때, ' +
          '<code>data[:, :1].shape</code> 와 <code>data[:, 0].shape</code> 는 각각 무엇인가요?',
        choices: [
          '둘 다 <code>(100836,)</code> — 어차피 같은 숫자가 나오니 모양도 같아요',
          ...
          '둘 다 <code>(100836, 1)</code> — 열 하나를 뽑았으니 열 축은 남아요'
        ],
        answer: 1,
        explain: '뽑히는 <b>숫자는 같지만 shape 가 달라요.</b> 규칙은 하나예요 — ' +
          '<b>정수 인덱스는 그 축을 없애고, 슬라이스는 그 축을 길이 1로 남겨요.</b> ' +
          '<code>:1</code> 은 슬라이스라서 열 축이 남아 2차원, <code>0</code> 은 정수라서 열 축이 사라져 1차원이에요. ' +
          '수업 셀 5 는 <code>data[:, :1]</code> 을 쓰므로 결과가 2차원이라는 점을 알고 있어야 해요.'
      },
      {
        q: '<code>np.array([200, 100]).astype(np.int8)</code> 의 결과는?',
        choices: [
          '<code>array([127, 100], dtype=int8)</code> — 범위를 넘으면 최댓값으로 잘려요',
          '<code>array([-56, 100], dtype=int8)</code>',
          '<code>array([200, 100], dtype=int8)</code> — dtype 만 바뀌고 값은 그대로예요',
          '<code>OverflowError</code> 가 나서 실행이 멈춰요'
        ],
        answer: 1,
        explain: 'int8 은 8비트라 서로 다른 값 2<sup>8</sup> = 256 가지만 구별해요. ' +
          '200 을 256 으로 나눈 나머지는 200 이고, 이것이 128 이상이므로 음수 쪽으로 넘어가 ' +
          '<b>200 − 256 = −56</b> 이 돼요. 잘리는(clip) 것이 아니라 <b>시계처럼 한 바퀴 도는(wrap)</b> 것이에요. ' +
          '더 무서운 것은 <code>astype</code> 이 <b>경고도 없이</b> 이렇게 한다는 점이에요. ' +
          '<code>OverflowError</code> 는 <code>np.array(200, dtype=np.int8)</code> 처럼 ' +
          '파이썬 정수를 직접 넣을 때 NumPy 2.0 이 내는 예외로, 경우가 달라요.'
      },
      {
        ...
        answer: 1,
        explain: '<code>np.round</code> 는 <b>은행가 반올림</b>이에요 — .5 는 <b>짝수 쪽</b>으로 보내요. ' +
          '그래서 2.5 → 2.0(2가 짝수), 3.5 → 4.0(4가 짝수)이에요. ' +
          '<code>astype(int)</code> 은 반올림이 아니라 <b>버림</b>이므로 2.7 → 2 예요. ' +
          '학교에서 배운 「.5 는 올린다」를 그대로 적용하면 첫 값에서 반드시 틀려요.'
      },
      {
        q: '파이썬에서 <code>0.1 + 0.2 == 0.3</code> 을 실행하면?',
        choices: [
          '<code>True</code> — 수학적으로 같으니 컴퓨터도 같다고 봐요',
          '<code>False</code> — 0.1 과 0.2 를 2진수로 정확히 담을 수 없어 아주 작은 오차가 남아요',
          '<code>False</code> — 파이썬은 실수끼리의 <code>==</code> 를 항상 False 로 처리해요',
          '오류가 나요. 실수는 <code>np.isclose</code> 로만 비교할 수 있어요'
        ],
        answer: 1,
        explain: '0.1 과 0.2 는 2진 소수로 쓰면 끝나지 않는 소수라서 float64 는 근삿값만 저장해요. ' +
          '그 결과 <code>0.1 + 0.2</code> 는 <b>0.30000000000000004</b> 가 되고, ' +
          '<code>0.3</code> 과 약 5.55×10<sup>−17</sup> 만큼 달라요. ' +
          '실수끼리의 <code>==</code> 자체가 금지된 것은 아니지만(<code>0.5 + 0.25 == 0.75</code> 는 True 다), ' +
          '<b>언제 맞고 언제 틀릴지 예측할 수 없으므로 쓰지 않는 것이 원칙</b>이에요. ' +
          '<code>np.isclose</code>, <code>np.allclose</code> 를 써요.'
      }
    ], { id: 'ndarray' }));
  }
  ...
  var mod = {
    ...
    blurb: '기온 한 값부터 4차원 컬러 이미지 묶음까지, 데이터를 shape 로 바라보는 법을 익히고 ' +
      'dtype 이 값을 조용히 바꿔 버리는 세 가지 순간을 직접 확인해요.',
    sim: '차원 탐험기 · 열 뽑기 · 가짜 손글씨 픽셀 탐험기 · dtype 실험실 · 속성 계산기',
    render: function (root) {
      root.appendChild(p('배열을 처음 만나면 세 가지를 물어야 해요 — ' +
        '<b>축이 몇 개인가(ndim), 각 축이 얼마나 긴가(shape), 칸 하나에 무엇이 들었나(dtype).</b> ' +
        '이 장은 그 세 질문을 실제 데이터 두 개(영화 평점 · 손글씨 이미지)로 끝까지 따라가요. ' +
        'dtype 은 특히 조심해야 해요 — <b>틀린 답을 오류 없이 조용히 내놓는 유일한 부분</b>이기 때문이에요.'));
      secDims(root);
      secRatings(root);
      secMnist(root);
      secDtype(root);
