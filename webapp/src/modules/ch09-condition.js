...
(function () {
  ...
  function simChecker() {
    ...
    function rebuild() {
      ...
      view.appendChild(el('p', { class: 'small muted', html:
        '마스크의 shape 는 ' + ND.shapeStr(mask.shape) + ' — 원본과 똑같아요. dtype 은 <b>' +
        mask.dtype + '</b>. 조건은 참·거짓 하나가 아니라 <b>배열</b>로 돌아와요.' }));
      ...
      if (axisNum === null) {
        ...
        try {
          ...
          view.appendChild(UI.callout('why',
            '1차원 배열에는 축이 <code>axis=0</code> 하나뿐이에요. 없는 축을 부르면 NumPy 는 ' +
            '그대로 <b>AxisError</b> 를 내요. 배열 <b>arr</b> 을 관절염 6×8 로 바꾸면 ' +
            'axis=1 도 살아나요.'));
          return;
        }
        view.appendChild(el('div', { class: 'flow' }, [
        ...
        view.appendChild(el('p', { class: 'small muted', html:
          '축을 지정하면 그 축이 <b>사라져요</b>: ' + ND.shapeStr(mask.shape) + ' → ' +
          ND.shapeStr(okAll.shape) + '. axis=1 은 "행마다 한 칸", axis=0 은 "열마다 한 칸" 이에요.' }));
    ...
    return UI.card({
      ...
      note: '조건을 만들면 마스크가 생기고, <b>np.all</b> 과 <b>np.any</b> 가 그 마스크를 하나의 ' +
            '참·거짓으로 줄여요. 조건을 깬 칸은 빨강으로 표시돼요. axis 를 바꾸면 행별·열별 검사가 돼요.',
      body: [ctl, view]
    });
  }
  ...
  function simCombiner() {
    ...
    function rebuild() {
      ...
      view.appendChild(el('p', { class: 'small muted', html:
        '마지막 줄이 조건에 걸린 환자들의 40일 평균 염증 수치예요. 참고로 16명 전체 평균은 <b>' +
        sc(ND.mean(sub)) + '</b> 이에요.' }));
    }
    ...
    var traps = el('div', { class: 'chips' }, [
      UI.btn('c1 and c2 로 쓰면?', function () {
        ...
        errHost.appendChild(UI.callout('why',
          '파이썬의 <code>and</code> 는 왼쪽 값을 <b>하나의 참·거짓</b>으로 판정하려 해요. ' +
          '그런데 c1 은 원소가 ' + N + '개인 배열이에요. "16개가 참이냐"는 질문에 답이 없으므로 ' +
          'NumPy 는 거부하고, <code>.all()</code> 이나 <code>.any()</code> 로 줄이라고 알려 줘요. ' +
          '원소별로 묶고 싶으면 <b>&</b> 를 써야 해요.'));
      }),
      UI.btn('괄호를 빼면?', function () {
        ...
        errHost.appendChild(UI.callout('trap',
          '<code>&</code> 는 비교 연산자보다 <b>우선순위가 높아요</b>. 그래서 파이썬은 이 식을 ' +
          '<code>arr6 &gt; (2 &amp; arr6) &lt; 5</code> 로 읽고, 이어서 연쇄 비교로 풀어 ' +
          '<code>and</code> 를 쓰다가 위 에러를 내요. 실수 배열이라면 <code>2 &amp; arr</code> ' +
          '자체가 불가능해 <b>TypeError</b>(bitwise_and not supported) 가 나요. ' +
          '조건마다 괄호를 씌우는 것은 선택이 아니라 <b>필수</b>예요.'));
      }),
    ...
    return UI.card({
      kicker: '시뮬레이터',
      title: '조건 조합기 — & 와 | 로 두 조건을 묶어요',
      note: '관절염 데이터 앞 16명을 대상으로, 서로 다른 두 날짜(열)에 조건을 걸어 봐요. ' +
            '조건 1(파랑)·조건 2(주황)·결합(초록) 마스크가 나란히 갱신돼요. ' +
            '아래 버튼은 학생들이 가장 많이 내는 두 에러를 실제 메시지로 보여 줘요.',
  ...
  function simWhere() {
    ...
    function parseSide(s, a) {
      s = String(s).trim();
      if (s === '') throw new ND.NDError('빈 칸은 값이 될 수 없어요.');
      if (s === 'a') return a;
      ...
      throw new ND.NDError("여기에는 숫자나 a, a*2, a+5, -a 같은 식만 쓸 수 있어요: '" + s + "'");
    }
    ...
    function rebuild() {
      ...
      if (st.mode === '1') {
        ...
        view.appendChild(UI.callout('trap',
          '결과 맨 끝의 쉼표를 보세요 — <code>' + UI.esc(tup) + '</code> 는 배열이 아니라 ' +
          '<b>배열이 든 튜플</b>이에요. 축이 ' + mask.ndim + '개니까 배열도 ' + mask.ndim +
          '개 들어 있어요. 그래서 실제로 인덱스를 쓰려면 거의 항상 ' +
          '<code>np.where(...)[0]</code> 처럼 꺼내 써야 해요.'));
        return;
      }
      ...
      view.appendChild(UI.legend([
        { color: 'var(--s1)', label: '조건이 참 → 참값(' + st.tv + ') 에서 가져왔어요' },
        { color: 'var(--s2)', label: '조건이 거짓 → 거짓값(' + st.fv + ') 에서 가져왔어요' }
      ]));
      ...
      view.appendChild(el('p', { class: 'small muted', html:
        ...
        ND.shapeStr(mask.shape) + ' 만큼 늘어나요 — 그래서 숫자와 배열을 섞어 써도 돼요.' }));
    }
    ...
    return UI.card({
      kicker: '시뮬레이터',
      title: 'where 시각화 — 인덱스도 되고 값도 돼요',
      note: '<b>인자 1개</b>면 조건이 참인 자리의 <b>인덱스(튜플)</b>, <b>인자 3개</b>면 조건에 따라 ' +
            '값을 고른 <b>새 배열</b>이 나와요. 참값·거짓값 칸에는 숫자뿐 아니라 ' +
            '<code>a</code>, <code>a*2</code>, <code>a+5</code>, <code>-a</code> 도 쓸 수 있어요.',
  ...
  function simNanLab() {
    ...
    function rebuild() {
      ...
      view.appendChild(el('p', { class: 'small', html:
        '칸을 <b>클릭</b>하면 그 값이 nan(측정 실패)이 돼요. 다시 누르면 되돌아와요. ' +
      ...
      if (nNan === 0) {
        view.appendChild(UI.callout('tip',
          'nan 이 하나도 없으면 두 줄의 값이 같아요. 이제 위 격자에서 칸 하나만 눌러 보세요 — ' +
          '<b>왼쪽 열 전체가 한꺼번에 nan 이 돼요.</b>'));
      } else if (clean.size === 0) {
        view.appendChild(UI.callout('trap',
          '전부 nan 이면 걸러낼 값이 남지 않아요. 이때 <code>np.nansum</code> 은 <b>0.0</b> 을 ' +
          '돌려주지만 <code>np.nanmean</code> 은 nan + RuntimeWarning, ' +
          '<code>np.nanmax</code> 는 아예 <b>ValueError</b> 를 내요. ' +
          '"결측치를 무시해요"는 말이 "결측치가 없는 것처럼 돼요"는 뜻은 아니에요.'));
      } else {
        view.appendChild(UI.callout('why',
          'nan 은 <b>어떤 수와 연산해도 nan</b> 이에요. 합에 nan 이 한 번 들어오면 그 뒤 모든 덧셈이 ' +
          'nan 이고, 평균·표준편차·최댓값까지 전부 nan 이 돼요. 2400개 중 하나만 빠져도 ' +
          '통계 전체가 무의미해지는 것이에요. 그래서 <b>nan 을 먼저 처리</b>하고 계산해야 해요. ' +
          '남은 ' + clean.size + '개로 계산한 합은 <b>' + sc(ND.sum(clean)) + '</b> 이에요.'));
      }
    }
    ...
    return UI.card({
      kicker: '시뮬레이터',
      title: 'nan 전파 실험실 — 한 칸이 전체를 무너뜨려요',
      note: '관절염 데이터 0번 환자의 day 15~22 값이에요. 칸을 눌러 결측치를 만들고, ' +
            '보통 함수와 <code>nan*</code> 함수의 결과가 어떻게 갈라지는지 보세요. ' +
            'nan 을 건너뛴 값은 <code>a[~np.isnan(a)]</code> 로 걸러낸 배열에서 실제로 계산한 것이에요.',
      body: [view]
    });
  }
  ...
  function render(root) {
    ...
    root.appendChild(el('p', { class: 'lede', html:
      'NumPy 에서 조건을 쓰면 <b>참·거짓 하나</b>가 아니라 <b>참·거짓 배열</b>이 돌아와요. ' +
      '이 배열을 마스크(mask)라 부르고, 마스크를 줄이고 조합하고 값으로 바꾸는 도구가 ' +
      'all·any·where 예요. 마지막에는 실제 데이터에 반드시 섞여 있는 결측치 nan 을 다뤄요.' }));
    ...
    root.appendChild(el('p', { html:
      '<code>np.all</code> 은 "전부 참인가", <code>np.any</code> 는 "하나라도 참인가"를 물어요. ' +
      '조건 배열을 넣으면 그 배열을 하나의 참·거짓으로 줄여 줘요. 수업 노트북의 예를 그대로 계산해 봐요.' }));
    ...
    root.appendChild(el('p', { html:
      '조건이 아니라 배열을 그냥 넣어도 동작해요. 이때는 <b>0 을 거짓, 0 이 아닌 값을 참</b>으로 봐요. ' +
      '<code>np.arange(10)</code> 에는 0 이 들어 있으니 결과가 <b>False</b> 다 — ' +
      '수업 시간에 가장 많이 놀라는 지점이에요.' }));
    ...
    root.appendChild(el('p', { class: 'small muted', html:
      '왼쪽 빨간 칸 하나(값 0)가 결과를 False 로 만들어요. 시작을 1 로 바꾸면 True 가 돼요.' }));

    var emptyArr = ND.array([], 'float64');
    root.appendChild(UI.table([
      { k: 'e', label: '식' }, { k: 'd', label: '왜' }, { k: 'v', label: '결과', num: true }
    ], [
      { e: 'np.all(np.array([1, 2, 3]))', d: '0 이 없음', v: sc(ND.all(ND.array([1, 2, 3]))) },
      { e: 'np.all(np.array([0, 1, 2]))', d: '0 이 하나 있음 → 거짓 취급', v: sc(ND.all(ND.array([0, 1, 2]))) },
      { e: 'np.all(np.array([-3, 2, 7]))', d: '음수도 0 이 아니면 참', v: sc(ND.all(ND.array([-3, 2, 7]))) },
      { e: 'np.any(np.zeros(5))', d: '전부 0', v: sc(ND.any(ND.zeros(5))) },
      { e: 'np.all(np.array([]))', d: '빈 배열 — 조건을 깰 원소가 없어요', v: sc(ND.all(emptyArr)) },
      { e: 'np.any(np.array([]))', d: '빈 배열 — 만족하는 원소가 없어요', v: sc(ND.any(emptyArr)) }
    ]));
    root.appendChild(UI.callout('why',
      '빈 배열의 <code>all</code> 이 True 인 게 이상해 보이지만 논리적으로는 당연해요. ' +
      '"모든 원소가 조건을 만족해요"의 반대는 "조건을 깨는 원소가 하나라도 있어요"인데, ' +
      '원소가 없으면 깨는 원소도 없어요. 수학에서 말하는 <b>공허한 참(vacuous truth)</b> 이에요. ' +
      '데이터를 걸러낸 뒤 빈 배열이 남았는데 all 이 True 라서 통과해 버리는 버그가 실제로 자주 나요.'));
    root.appendChild(UI.callout('trap',
      '파이썬 내장 <code>all()</code>·<code>any()</code> 와 이름은 같지만 다른 함수예요. ' +
      '내장 함수는 원소를 하나씩 꺼내 참·거짓을 판정하므로, 2차원 배열에 쓰면 ' +
      '"행 하나"를 판정하려다 <b>ValueError</b> 를 내요. 게다가 파이썬 반복문으로 돌아 훨씬 느려요. ' +
      '배열에는 <b>np.all / np.any</b> 나 <code>arr.all() / arr.any()</code> 를 써요.'));

    root.appendChild(simChecker());

    if (DATA) {
      root.appendChild(el('h3', { class: 'h-sub', text: '실전: 안전했던 환자를 찾아 보세요' }));
      root.appendChild(el('p', { html:
        '관절염 데이터는 환자 60명 × 40일이에요. "40일 동안 <b>한 번도</b> 염증이 15를 넘지 않은 환자"는 ' +
        '행마다 all 을 걸어 찾아요. 축을 잘못 고르면 전혀 다른 질문이 돼요.' }));
      root.appendChild(UI.code(
      ...
      root.appendChild(UI.callout('trap',
        '<code>axis=1</code> 은 "각 <b>환자</b>에 대해 40일을 훑어요", <code>axis=0</code> 은 ' +
        '"각 <b>날짜</b>에 대해 60명을 훑어요"는 뜻이에요. 사라지는 축이 곧 훑는 축이에요. ' +
        '환자를 세려면 남아야 할 축이 환자 축(axis 0)이므로 지워야 할 축은 axis=1 이에요.'));
    }
    ...
    root.appendChild(el('p', { html:
      '비교 연산자는 <b>원소끼리</b> 짝지어 비교하고, 같은 shape 의 bool 배열을 돌려줘요. ' +
      '한쪽이 숫자면 브로드캐스팅으로 늘어나 모든 원소와 비교돼요.' }));
    root.appendChild(UI.code(
    ...
    root.appendChild(UI.errBlock(
      ...
      'Use a.any() or a.all()', 'ValueError'));
    root.appendChild(UI.callout('why',
      '<code>if</code> 는 조건을 <b>참 또는 거짓 하나</b>로 판정해야 해요. 그런데 ' +
      '<code>temp1 &gt; temp2</code> 는 ' + ND.format(ND.ops.gt(temp1, temp2)) +
      ' 처럼 원소가 3개예요. 첫 원소만 볼지, 전부 참이어야 하는지, 하나라도 참이면 되는지 ' +
      'NumPy 는 알 수 없어요. 그래서 <b>추측하지 않고 에러를 내요.</b> ' +
      '뜻을 정해서 <code>.all()</code> 이나 <code>.any()</code> 로 직접 줄여 줘야 해요. ' +
      '(원소가 1개인 배열은 예외적으로 통과하지만, 그 동작에 기대는 코드는 위험해요.)'));

    root.appendChild(el('h3', { class: 'h-sub', text: '배열 두 개가 같은지 비교하기' }));
    root.appendChild(el('p', { html:
      '<code>a == b</code> 는 배열을 돌려주니 "두 배열이 같은가"라는 질문의 답이 아니에요. ' +
      '정수라면 <code>np.array_equal</code>, <b>실수라면 반드시 <code>np.allclose</code></b> 를 써요.' }));
    ...
    root.appendChild(UI.callout('trap',
      '컴퓨터는 실수를 2진 소수로 저장하는데 0.1 이나 0.7 은 2진법으로 딱 끊어지지 않아요. ' +
      '그래서 <code>0.1 + 0.2</code> 는 0.3 이 아니라 ' + String(0.1 + 0.2) + ' 가 돼요. ' +
      '화면에는 <code>print</code> 가 반올림해 <b>0.3</b> 으로 보이니 더 헷갈려요. ' +
      '<b>실수 배열을 <code>==</code> 로 비교하지 마세요.</b> 언제나 <code>np.allclose</code> 예요.'));

    root.appendChild(el('h3', { class: 'h-sub', text: '논리 연산자 — 괄호를 잊지 마세요' }));
    root.appendChild(UI.table([
    ...
    root.appendChild(el('p', { class: 'small muted', html:
      ...
      '실제 계산한 값이에요. <code>&amp;</code> 의 결과가 3, 4, 5 자리에서만 True 인 것을 확인하세요.' }));
    root.appendChild(UI.callout('tip',
      '<code>and / or / not</code> 은 파이썬 문법이라 배열에 못 써요. 배열에는 ' +
      '<code>&amp; | ~ ^</code> 를 쓰고, 조건마다 <b>괄호</b>를 씌워요: ' +
      '<code>(a &gt; 2) &amp; (a &lt; 6)</code>. ' +
      '<code>~</code> 는 뺄셈이 아니라 부정이에요 — <code>a[~np.isnan(a)]</code> 처럼 아주 자주 써요.'));
    ...
    root.appendChild(el('p', { html:
      '<code>np.where</code> 는 인자 개수에 따라 <b>완전히 다른 일</b>을 해요. ' +
      '인자가 1개면 조건이 참인 자리의 <b>인덱스</b>, 3개면 조건에 따라 값을 고른 <b>배열</b>이에요. ' +
      '이 둘을 섞어 쓰는 실수가 흔해요.' }));
    ...
    root.appendChild(UI.callout('trap',
      '<code>np.where(cond)</code> 의 결과에는 <b>괄호와 쉼표</b>가 붙어 있어요 — ' +
      '<code>' + UI.esc(whereTupleStr(m35)) + '</code>. 배열이 아니라 <b>튜플</b>이기 때문이에요. ' +
      '왜 튜플인가요? 2차원 배열이면 행 인덱스 배열과 열 인덱스 배열이 <b>둘</b> 필요해요. ' +
      '축 개수만큼 배열을 담으려고 튜플로 감싼 것이에요. ' +
      '1차원에서도 규칙이 같으므로 원소 1개짜리 튜플이 나오고, 그래서 ' +
      '<code>np.where(...)[0]</code> 로 꺼내 써요.'));
    ...
    root.appendChild(UI.callout('why',
      '첫 원소가 0 인 이유는 단순해요. temp4[0] 은 <b>10</b> 이고 조건은 <code>&gt; 10</code> ' +
      '(초과)이므로 <code>10 &gt; 10</code> 은 거짓이에요. 거짓이면 세 번째 인자인 0 을 가져와요. ' +
      '경계값을 포함하려면 <code>&gt;=</code> 를 써야 해요: ' +
    ...
    root.appendChild(UI.code(
      ...
      '# np.select 는 같은 일을 납작하게 써요 (조건은 위에서부터 먼저 맞는 것 하나만)\n' +
    ...
    root.appendChild(el('p', { class: 'small muted', html:
      'where 를 3겹으로 겹치면 읽기가 힘들어져요. 조건이 3개 이상이면 <code>np.select</code> 가 나아요. ' +
      '두 방법 모두 결과는 ' + ND.format(gp) + ' 로 같아요.' }));
    ...
    root.appendChild(UI.code(
      ...
      'np.clip(raw, 5, 15)             # 위아래 한꺼번에 — 이게 훨씬 짧아요'));
    root.appendChild(UI.out(
      'raw                        → ' + ND.format(raw) + '\n' +
      'np.where(raw > 15, 15, raw) → ' + ND.format(upperOnly) + '\n' +
      'np.clip(raw, 5, 15)         → ' + ND.format(both) + '\n' +
      '  (위 clip 값은 where 를 두 번 겹쳐 실제로 계산한 것이에요 — 결과가 같아요)'));
    root.appendChild(UI.callout('tip',
      '아래·위를 동시에 자르는 일은 <code>np.clip(a, min, max)</code> 한 줄로 끝나요. ' +
      'where 는 <b>자르기 말고 다른 값으로 바꿀 때</b> 써요 — 예를 들어 이상치를 평균으로 채우거나 ' +
      'nan 으로 바꿔 두고 나중에 처리할 때예요.'));
    ...
    if (DATA) {
      ...
      root.appendChild(UI.out(
        ...
        '(data >= 15).sum()        → ' + sc(ND.sum(ND.ops.ge(DATA, 15))) +
          '   ← bool 배열의 합은 True 의 개수예요'));
      root.appendChild(UI.callout('tip',
        'bool 배열을 <code>sum</code> 하면 True 가 1로 세어져요. 그래서 ' +
        '<code>np.where(cond, 1, 0).sum()</code> 과 <code>cond.sum()</code> 은 같은 값이에요. ' +
        '개수만 필요하면 where 없이 <code>cond.sum()</code> 이 더 짧아요.'));
      ...
      root.appendChild(el('p', { class: 'small muted', html:
        '앞 12명의 염증 수치예요. 색이 진할수록 높고, <b>주황 외곽선</b>이 ' +
        '<code>data &gt;= 15</code> 인 칸 — 위 마스크가 실제로 잡아낸 자리예요.' }));
    }
    ...
    root.appendChild(UI.code(
      'b = np.array([[1, 20],\n' +
      '              [16, 3]])\n' +
      'np.nonzero(b >= 15)    # np.where(b >= 15) 와 똑같아요 (축별 인덱스 튜플)\n' +
    ...
    root.appendChild(UI.callout('tip',
      '셋을 구분하는 기준은 <b>결과의 모양</b>이에요. <code>where</code>·<code>nonzero</code> 는 ' +
      '"축별 인덱스 배열들의 튜플"이라서 곧바로 <code>b[...]</code> 인덱싱에 넣을 수 있고, ' +
      '<code>argwhere</code> 는 "좌표 목록"이라서 <code>for r, c in np.argwhere(...)</code> ' +
      '처럼 하나씩 돌 때 편해요. <code>select</code> 는 조건이 여러 개일 때의 where 예요.'));
    ...
    root.appendChild(UI.callout('ver',
      '수업자료의 <code>np.array([1, np.NaN, np.Inf])</code> 는 <b>지금 실행하면 에러가 나요.</b> ' +
      'NumPy 2.0(2024년 6월)에서 대문자 별칭이 삭제되었기 때문이에요. ' +
      '<code>np.nan</code>, <code>np.inf</code> 처럼 <b>전부 소문자</b>로 써야 해요. ' +
      '옛 블로그·강의자료·인터넷 코드에서 <code>np.NaN</code>, <code>np.Inf</code>, ' +
      '<code>np.float_</code>, <code>np.int</code> 를 보면 그 자리에서 소문자 이름으로 고쳐 읽어 보세요.'));
    ...
    root.appendChild(UI.callout('why',
      ...
      '헷갈리기까지 했어요. 2.0 은 "이름 하나에 뜻 하나"로 정리한 대청소였어요. ' +
      '덕분에 옛 코드가 깨지지만, 에러 메시지가 <b>고칠 이름을 직접 알려 줘요</b>는 점은 친절해요.'));

    root.appendChild(el('h3', { class: 'h-sub', text: 'nan 이 섞이면 dtype 이 바뀌어요' }));
    ...
    root.appendChild(el('p', { html:
      '정수 1 을 넣었는데도 dtype 이 <b>float64</b> 예요. nan 과 inf 는 <b>실수(IEEE 754)</b> 에만 ' +
      '존재하는 특별한 비트 패턴이라서, 하나라도 섞이면 배열 전체가 실수로 승격돼요.' }));
    root.appendChild(UI.code(
    ...
    root.appendChild(UI.callout('trap',
      '<b>정수 배열에는 결측치를 표시할 방법이 없어요.</b> 실제 데이터를 다룰 때 이것이 큰 제약이에요. ' +
      '결측치가 생길 수 있는 열은 처음부터 <code>float64</code> 로 읽어야 해요. ' +
      '<code>ints.astype(float)</code> 로 바꾼 뒤에 nan 을 넣으면 돼요. ' +
      '(정수 결측치를 정말 다뤄야 하면 pandas 의 nullable 정수형이나 마스크 배열을 써요.)'));
    ...
    root.appendChild(el('p', { class: 'small muted', html:
      '<code>isfinite</code> 는 nan 과 inf 를 <b>둘 다</b> 걸러요. ' +
      '"쓸 수 있는 값만 남기고 싶어요"면 <code>isnan</code> 이 아니라 <code>isfinite</code> 가 맞아요.' }));

    root.appendChild(el('h3', { class: 'h-sub', text: 'nan 은 자기 자신과도 같지 않아요' }));
    ...
    root.appendChild(UI.callout('trap',
      '<b>nan == nan 은 False 예요.</b> nan 은 "값이 없어요"는 표시이지 값이 아니에요. ' +
      '측정 실패 두 개가 서로 같다고 말할 근거가 없으니 IEEE 754 표준이 아예 ' +
      '"nan 과의 모든 비교는 거짓"으로 정했어요. 반면 inf 는 정상적인 값이라 ' +
      '<code>inf == inf</code> 는 True 예요. ' +
      '따라서 <code>a[a == np.nan]</code> 은 <b>영원히 빈 배열</b>이에요 — ' +
      '반드시 <code>a[np.isnan(a)]</code> 를 써야 해요. ' +
      '<code>np.isnan</code> 이 존재하는 이유가 바로 이것이에요.'));

    root.appendChild(simNanLab());

    root.appendChild(el('h3', { class: 'h-sub', text: '0 으로 나누면 — 파이썬과 NumPy 가 달라요' }));
    ...
    root.appendChild(UI.callout('why',
      'NumPy 는 <b>멈추지 않아요.</b> 배열 100만 개를 계산하는 중에 한 칸이 0 나누기라고 ' +
      '전체를 중단시키면 손해가 크기 때문이에요. 대신 <code>1/0 → inf</code>, ' +
      '<code>0/0 → nan</code>(정의 자체가 불가능) 으로 채우고 <b>RuntimeWarning</b> 만 띄워요. ' +
      '경고는 기본적으로 <b>같은 자리에서 한 번만</b> 나오니, 무시하고 지나치기 쉬워요.'));
    root.appendChild(UI.code(
      "# 경고 다루기\n" +
    ...
    root.appendChild(UI.callout('tip',
      '경고를 끄는 것은 "문제가 없어요"가 아니라 "내가 이미 알고 있고 뒤처리도 해요"는 선언이에요. ' +
      '<code>errstate</code> 로 껐다면 반드시 <code>isfinite</code> 로 뒤처리하세요. ' +
      '디버깅 중이라면 <code>np.seterr(all=\'raise\')</code> 로 <b>경고를 예외로 바꿔</b> ' +
      'nan 이 처음 생기는 지점을 잡는 것이 훨씬 빨라요.'));

    root.appendChild(UI.callout('tip',
      '실전에서 nan 은 대개 <b>측정 실패·응답 없음·기록 누락</b>을 뜻해요. 관절염 데이터라면 ' +
      '"그날 환자가 오지 않았어요", 설문이라면 "무응답"이에요. 그러니 nan 을 만나면 먼저 물어야 해요 — ' +
      '<b>왜 비었나요?</b> 무응답이 특정 집단에 몰려 있다면 그 자체가 데이터예요. ' +
      '0 으로 채우는 것이 가장 위험해요: 없는 값이 "0 이라는 측정값"으로 바뀌어 평균을 끌어내려요.',
      'nan 을 만나면'));
    ...
    root.appendChild(el('p', { html:
      'nan 은 대부분 <b>파일을 읽는 순간</b> 들어와요. CSV 에 빈칸이 하나 있으면 ' +
      '<code>np.loadtxt</code> 는 아예 실패하고, <code>np.genfromtxt</code> 는 그 자리를 nan 으로 채워요. ' +
      '이 차이가 두 함수를 구분하는 이유예요.' }));
    ...
    root.appendChild(UI.out(
      D && D.inflammationMeta
        ? "np.loadtxt('lab_inflammation-01.csv', delimiter=',')\n" +
          ...
          ' · 헤더 없음 → skiprows 불필요'
        : '(관절염 데이터가 이 빌드에 포함되지 않았어요)'));
    root.appendChild(UI.callout('tip',
      '<code>skiprows</code> 는 <b>맨 위 몇 줄을 버릴지</b>예요. 관절염 데이터는 헤더가 없으니 0(기본값), ' +
      '영화 평점 <code>ra.csv</code> 는 <code>' +
      UI.esc((D && D.ratingsMeta && D.ratingsMeta.header) || 'userId,movieId,rating,timestamp') +
      '</code> 라는 헤더가 한 줄 있으니 <code>skiprows=1</code> 이에요. ' +
      '빼먹으면 문자열을 실수로 바꾸다 실패해요. ' +
      '<code>fmt=\'%.2f\'</code> 는 저장할 때의 소수점 자릿수예요 — 빼면 지수 표기로 길게 저장돼요.'));
    ...
    root.appendChild(UI.callout('trap',
      '<code>loadtxt</code> 가 실패하는 것이 <b>단점이 아니에요.</b> "이 파일에는 내가 예상 못한 것이 ' +
      '들어 있어요"고 큰 소리로 알려 주는 것이에요. <code>genfromtxt</code> 로 바꿔 조용히 nan 을 ' +
      '받아들이는 순간, 그 nan 이 어디에 몇 개 있는지 <b>내가 직접 세어야 하는 책임</b>이 생겨요. ' +
      '읽은 직후에 <code>np.isnan(data).sum()</code> 을 찍어 보는 습관을 들여 보세요.'));
    ...
    root.appendChild(UI.callout('trap',
      ...
      '<b>붙여 주지 않아요.</b> 이 비대칭 때문에 "저장은 됐는데 못 불러와요"는 일이 자주 생겨요. ' +
    ...
    root.appendChild(UI.table([
      ...
      { k: '사람이 열어 볼 수 있나', t: '돼요 — 엑셀·메모장에서 그대로', b: '안 돼요 — 깨진 글자로 보여요' },
      { k: 'dtype 보존', t: '안 돼요 — 읽으면 기본 float64', b: '돼요 — int8 이든 bool 이든 그대로' },
      { k: 'shape 보존', t: '2차원까지만', b: '몇 차원이든 그대로' },
      { k: '용량', t: '커요 (숫자를 글자로 적어요)', b: '작아요 (메모리 그대로)' },
      { k: '속도', t: '느려요 (글자 ↔ 숫자 변환)', b: '빨라요' },
      { k: '다른 프로그램과 공유', t: '쉬워요 — 사실상 표준', b: '어려워요 — NumPy 전용' },
      { k: '언제 쓰나', t: '결과를 남기거나 남에게 줄 때', b: '내 계산 중간 결과를 다시 쓸 때' }
    ]));
    root.appendChild(UI.callout('trap',
      '<code>np.load(..., allow_pickle=True)</code> 는 <b>파일 안의 파이썬 객체를 되살리는</b> 옵션이에요. ' +
      '그 과정에서 <b>임의의 코드가 실행될 수 있어요</b> — 신뢰할 수 없는 곳에서 받은 ' +
      '<code>.npy</code>·<code>.npz</code> 에는 절대 쓰지 마세요. NumPy 가 이 옵션을 ' +
      '기본으로 끈 이유가 보안이에요. 순수한 숫자 배열이라면 이 옵션이 필요하지 않아요.',
      '보안 주의'));

    root.appendChild(UI.callout('ver',
      '수업 노트북 셀 156 은 <code>np.loadtxt(\'ratings.csv\', ...)</code> 라고 쓰지만, ' +
      '실습 폴더의 실제 파일명은 <code>ra.csv</code> 예요. 그대로 실행하면 아래 에러가 나요. ' +
      '파일을 못 찾는 에러의 90%는 <b>철자·확장자·작업 폴더</b> 셋 중 하나예요. ' +
      '<code>import os; os.listdir()</code> 로 지금 폴더에 무엇이 있는지 먼저 확인하세요.'));
    root.appendChild(UI.errBlock("ratings.csv not found.", 'FileNotFoundError'));
    if (D && D.ratingsMeta) {
      root.appendChild(UI.statRow([
        { k: '실제 파일명', v: D.ratingsMeta.file, sub: 'ratings.csv 가 아니에요' },
      ...
      root.appendChild(el('p', { class: 'small muted', html:
        '위 집계값은 <b>실제 전체 ' + D.ratingsMeta.trueShape[0] + '행</b>으로 정확히 계산한 사실이에요. ' +
        '다만 이 페이지에 실려 있는 ' + D.ratingsMeta.sampleRows + '행은 <b>합성 데이터</b>다 — ' +
        'MovieLens 라이선스가 재배포를 금지하기 때문이에요. 열 구성과 값의 범위만 실제와 같게 맞췄어요.' }));
    }
    ...
    root.appendChild(UI.quiz([
      {
        ...
        choices: ['True', 'False', 'array([False, True, True])', 'ValueError 가 나요'],
        answer: 1,
        explain: '조건 없이 배열만 넣으면 <b>0 은 거짓, 0 이 아닌 값은 참</b>으로 봐요. ' +
          '첫 원소가 0 이므로 "전부 참"이 아니어서 <b>False</b> 예요. ' +
          'np.all 은 배열을 하나의 참·거짓으로 <b>줄이는</b> 함수이므로 결과가 배열일 수 없어요. ' +
      ...
      },
      {
        ...
        answer: 1,
        explain: '인자가 1개인 where 는 <b>축마다 인덱스 배열 하나</b>를 만들어 <b>튜플</b>에 담아 줘요. ' +
          '1차원이라 배열 하나짜리 튜플이 되고, 그래서 끝에 쉼표가 붙어요: <code>' +
          UI.esc(whereTupleStr(m35)) + '</code>. ' +
          '보기 1(array 만)은 <code>np.where(...)[0]</code> 의 결과, ' +
          '보기 3(값)은 <code>temp4[temp4 &lt; 35]</code> 의 결과, ' +
          '보기 4(bool)는 <code>temp4 &lt; 35</code> 자체예요. 네 개가 전부 다른 것이에요.'
      },
      {
        q: '관절염 데이터 <code>data</code> 의 shape 는 (60, 40)(환자 × 날짜)이에요. ' +
        ...
        explain: '환자별로 답이 하나씩 나와야 하므로 결과 shape 는 (60,) — 즉 <b>날짜 축(axis=1)이 ' +
          '사라져야</b> 해요. <code>axis=0</code> 은 환자 축을 지워 (40,) 이 되므로 ' +
          '"날짜별로 60명 모두가 15 미만이었나"라는 다른 질문이에요. ' +
          '<code>any</code> 는 "하루라도 15 미만"이라 훨씬 느슨하고, ' +
          '<code>np.all(data &lt; 15)</code> 는 축이 없어 전체를 참·거짓 하나로 줄여요. ' +
          (DATA ? '실제로 정답 식으로 세어 보면 <b>' + sc(ND.sum(ND.all(ND.ops.lt(DATA, 15), 1))) +
            '명</b>(환자 ' + ND.format(ND.whereIdx(ND.all(ND.ops.lt(DATA, 15), 1))) +
            ')이고, any 로 세면 ' + sc(ND.sum(ND.any(ND.ops.lt(DATA, 15), 1))) + '명이 나와요.' : '')
      },
      {
        q: 'nan 이 섞인 실수 배열 <code>a</code> 에서 nan 인 칸을 찾으려 해요. ' +
           '<code>a[a == np.nan]</code> 은 어떻게 되나요?',
        choices: [
          'nan 인 칸만 골라져요',
          '항상 빈 배열이 나와요',
          'nan 을 포함한 배열 전체가 나와요',
          'ValueError 가 나요'
        ],
        answer: 1,
        explain: '<b>nan == nan 은 False</b> 다(실제 계산: ' +
          sc(ND.ops.eq(ND.array([NaN]), ND.array([NaN])).idx('0')) + '). ' +
          'nan 은 "값이 없어요"는 표시일 뿐이므로 자기 자신과도 같지 않다고 IEEE 754 가 정했어요. ' +
          '따라서 마스크가 전부 False 이고 결과는 <b>언제나 빈 배열</b>이에요 — ' +
          '에러도 나지 않아 조용히 틀린다는 점이 더 위험해요. ' +
          '반드시 <code>a[np.isnan(a)]</code> 를 쓰고, 반대로 성한 값만 남기려면 ' +
          '<code>a[~np.isnan(a)]</code> 또는 <code>a[np.isfinite(a)]</code> 를 써요.'
      }
    ], { id: 'condition' }));
  }
  ...
  Lab.register({
    ...
    n: '9',
    title: '조건, 논리, 결측치',
    blurb: '조건은 참·거짓 하나가 아니라 배열로 돌아와요. 그 마스크를 all·any 로 줄이고 &·|로 묶고 where 로 값을 고른 다음, 실제 데이터에 반드시 섞여 있는 nan 을 다뤄요.',
