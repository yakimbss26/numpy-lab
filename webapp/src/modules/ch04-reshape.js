...
(function () {
  ...
  function parseShapeStr(s) {
    ...
    if (!parts.length) throw new Error('목표 shape 가 비었어요. 정수를 콤마로 구분해 적어 보세요. 예: 3, 8');
    return parts.map(function (x) {
      if (!/^-?\d+$/.test(x)) {
        throw new Error("'" + x + "' 는 정수가 아니에요. shape 에는 정수만 써요. 예: 2, 3, 4");
      }
  ...
  function reshapeSim() {
    ...
    function resultBlock(src, shape, order, engineNd) {
      ...
      if (nd.ndim <= 3) {
        ...
        body.push(el('p', { class: 'small muted',
          text: '4차원 이상은 격자로 그리지 않아요. 아래는 print() 출력이에요.' }));
    ...
    function rebuild() {
      ...
      host.appendChild(panel('a', '원본 — 파랑. 칸을 누르면 그 원소를 노랑으로 추적해요', [
        UI.shapeBadge(src),
        UI.grid(src, {
          showIndex: true, cellSize: 36,
          label: function (idx, val) {
      ...
      if (negs === 1) {
        ...
        if (known > 0 && src.size % known === 0) {
          host.appendChild(UI.callout('why',
            ...
            ' = ' + (src.size / known) + ' → <b>-1 은 ' + (src.size / known) + ' 이 돼요.</b> ' +
            '나머지 한 축의 크기는 size 로부터 유일하게 정해지므로 NumPy 가 대신 계산해 줘요.',
            '-1 은 이렇게 정해져요'));
        }
      }
      ...
      try {
        ...
        var why;
        if (negs > 1) {
          why = '-1 은 "네가 계산하세요" 라는 뜻이에요. 두 곳에 쓰면 답이 하나로 정해지지 않으므로 ' +
                'NumPy 는 계산을 포기해요. -1 은 <b>한 번만</b> 쓸 수 있어요.';
        } else if (negs === 1) {
          var kn = 1;
          target.forEach(function (d) { if (d !== -1) kn *= d; });
          why = 'size ' + src.size + ' 를 지정한 ' + kn + ' 으로 나누면 ' +
                (src.size / kn).toFixed(4).replace(/0+$/, '') + ' — 정수가 아니에요. ' +
                '<b>size 가 나누어떨어지지 않으면 -1 도 소용없어요.</b> ' +
                '(수업자료 셀 40 이 바로 이 에러예요. 일부러 남겨 둔 예제예요.)';
        } else {
          why = '원소 개수는 늘거나 줄지 않아요. ' + ND.shapeStr(target) + ' 은 원소 ' +
                ND.prod(target) + '개가 필요한데 원본은 ' + src.size + '개뿐이에요. ' +
                '<b>reshape 의 유일한 조건은 size 가 같은 것</b>이에요.';
        }
      ...
      if (st.order === 'both') {
        ...
        host.appendChild(UI.callout('why',
          '같은 목표 shape 인데 원소 배치가 달라요. reshape 는 메모리를 옮기지 않아요 — ' +
          '<b>원소를 어떤 순서로 읽고 어떤 순서로 채울지</b>만 바꿔요. ' +
          'C 순서는 마지막 축이 가장 빨리 변하고(행을 다 채우고 다음 행), ' +
          'F 순서는 첫 축이 가장 빨리 변해요(열을 다 채우고 다음 열). ' +
          '"칸에 쓸 것" 을 <b>원본 평평한 인덱스</b>로 바꿔 보면 읽는 순서가 바로 보여요.'));
      } else {
        host.appendChild(panel('r', '결과 — 초록', resultBlock(src, res.shape, 'C', res)));
      }

      host.appendChild(el('p', { class: 'small', html:
        'size 는 <b>' + src.size + ' → ' + res.size + '</b> 로 그대로예요. ' +
        'np.shares_memory(' + SRC[st.src].name + ', 결과) → <b>' + (shared ? 'True' : 'False') +
        '</b> — ' + (shared
          ? 'reshape 는 값을 복사하지 않았어요. 같은 메모리를 다른 모양으로 보는 <b>뷰</b>예요.'
          : '원본이 연속(contiguous)이 아니어서 어쩔 수 없이 사본이 만들어졌어요.') }));
    ...
    return UI.card({
      kicker: '시뮬레이터 ①',
      title: 'reshape 시뮬레이터',
      note: '목표 shape 를 직접 적어 보세요. 칸을 누르면 그 원소를 <b>노랑</b>으로 추적해요 — ' +
            '어느 칸이 어디로 갔는지 눈으로 따라갈 수 있어요. ' +
            '아래 칩은 수업 노트북 셀 33~41 과 셀 61 을 그대로 재현한 것이에요.',
      body: [
  ...
  function flattenSim() {
    ...
    function rebuild() {
      ...
      if (st.track !== null) {
        ...
        host.appendChild(el('p', { class: 'small', html:
          ...
          'flatten 결과의 <b>' + st.track + '</b>번 칸으로 갔어요. ' +
          '평평한 위치 = ' + src[0] + '×' + cs[0] + ' + ' + src[1] + '×' + cs[1] +
          ' + ' + src[2] + '×' + cs[2] + ' = ' + st.track + '.' }));
      } else {
        host.appendChild(el('p', { class: 'small muted',
          text: '아무 칸이나 눌러 보세요. 대응하는 칸이 양쪽에서 노랑으로 켜져요.' }));
      }
    }
    ...
    return UI.card({
      ...
      note: '수업 셀 43~45 의 배열이에요. shape 는 <b>' + ND.shapeStr(m2.shape) + '</b>, ndim <b>' +
            m2.ndim + '</b> 이므로 flatten 결과 길이는 8 이 아니라 <b>' + flat.size +
            '</b> 이에요. 2×2×4 = ' + m2.size + ' 를 세어 보면 당연해요.',
  ...
  function viewLab() {
    ...
    function rebuild() {
      ...
      if (st.poked) {
        ...
        host.appendChild(UI.callout(aChanged ? 'trap' : 'tip',
          aChanged
            ? 'b 만 고쳤는데 <b>a 도 함께 바뀌었어요</b>(빨간 칸). b 는 a 와 같은 메모리를 보는 ' +
              '<b>뷰</b>였기 때문이에요. a 를 건드리지도 않았는데 값이 변했어요 — ' +
              '이것이 초보자를 가장 많이 괴롭히는 버그예요.'
            : 'b 를 고쳐도 <b>a 는 그대로예요</b>. b 는 값을 새 메모리에 복사해 둔 ' +
              '<b>사본</b>이라서 서로 영향이 없어요.'));
      }
      ...
      if (shared) {
        ...
        host.appendChild(el('p', { class: 'small', html:
          '<b>np.shares_memory(a, b) → False</b> — 버퍼가 두 개예요. 완전히 독립적이에요.' }));
      }
    ...
    return {
      card: UI.card({
        kicker: '시뮬레이터 ②',
        title: '뷰 vs 사본 실험실',
        note: '연산을 하나 고르고 <b>' + '아래 버튼' + '</b>을 눌러 b 의 첫 칸을 999 로 바꿔 보세요. ' +
              'a 가 함께 바뀌는지 안 바뀌는지가 뷰와 사본의 차이에요. ' +
              '이 장에서 딱 하나만 가져간다면 이것이에요.',
  ...
  function stridesSim() {
    ...
    function rebuild() {
      ...
      host.appendChild(panel('a', '이 창이 보는 값 — 칸을 눌러 보세요', [
        UI.shapeBadge(v),
        UI.grid(v, {
          cellSize: 38, showIndex: true,
      ...
      host.appendChild(UI.table(
        ...
        [
          ...
          { k: 'v.flags[\'C_CONTIGUOUS\']', v: v.isContiguous() ? 'True' : 'False',
            d: v.isContiguous() ? '메모리에 쭉 이어져 있어요 → reshape 가 뷰로 돼요'
                                : '건너뛰며 봐요 → reshape 는 사본을 만들어야 해요' }
        ]
      ));
    }
    ...
    return UI.card({
      ...
      note: '뷰가 마법이 아니라 <b>덧셈과 곱셈</b>이라는 것을 확인하는 도구예요. ' +
            '창을 바꿔 보면 값은 그대로인데 (shape, strides, offset) 세 숫자만 달라져요.',
  ...
  function axisSim() {
    ...
    function rebuild() {
      ...
      host.appendChild(el('p', { class: 'small', html:
        'size 는 ' + a.size + ' → ' + b.size + ' 로 그대로예요. 값도 하나도 안 바뀌었어요. ' +
        'ndim 만 ' + a.ndim + ' → ' + b.ndim + '. ' +
        'np.shares_memory(a, b) → <b>' + (ND.sharesMemory(a, b) ? 'True' : 'False') + '</b> — ' +
        '축을 추가하는 일은 <b>보는 방식만 바꾸는 것</b>이라 값을 복사하지 않아요.' }));
    ...
    return UI.card({
      ...
      note: '길이 4 인 1차원 배열에 축을 하나 끼워 넣어 봐요. ' +
            '<code>(4,)</code> 가 <code>(1, 4)</code> 인지 <code>(4, 1)</code> 인지가 ' +
            '7장 브로드캐스팅에서 결과를 완전히 갈라 놓아요.',
      body: [ctl, host]
    });
  }
  ...
  function transposeVsReshape() {
    ...
    function rebuild() {
      ...
      host.appendChild(el('p', { class: 'small muted',
        text: st.track === null
          ? '아무 칸이나 눌러 그 값이 세 격자에서 어디에 있는지 확인해 보세요.'
          : '값 ' + st.track + ' 의 자리를 노랑으로 표시했어요. .T 와 reshape 의 자리가 달라요.' }));
      host.appendChild(UI.table(
      ...
      host.appendChild(el('p', { class: 'small', html:
        'shape 는 둘 다 (3, 2) 지만 <b>값의 배치가 달라요.</b> ' +
        '.T 의 strides 는 ' + strTuple(t.strides) + ' 로 a 의 ' + strTuple(m.strides) +
        ' 를 뒤집은 것이고(메모리는 그대로), ' +
        'reshape 는 메모리에 놓인 순서대로 다시 끊어 읽은 것이에요. ' +
    ...
    return UI.card({
      kicker: '시뮬레이터 ⑤',
      title: '전치(.T) 와 reshape 는 달라요',
      note: '학생이 가장 많이 헷갈리는 지점이에요. shape 가 같아도 결과가 같지 않아요.',
      body: [host]
    });
  }
  ...
  function transpose3D() {
    ...
    box.appendChild(el('p', { class: 'small', html:
      ...
      '2번 축에는 원래 2번 축을 놓아 보세요" 는 뜻이에요. 축 0 과 1 만 맞바꾼 것이므로 ' +
      '<code>np.swapaxes(a, 0, 1)</code> 와 결과가 같아요. ' +
      'np.shares_memory(a, 결과) → <b>' + (ND.sharesMemory(a, t) ? 'True' : 'False') + '</b> ' +
      '— 전치는 언제나 뷰예요. 다만 연속성은 잃어요(a.transpose 결과가 연속인가 → <b>' +
  ...
  function ravelProof() {
    ...
    box.appendChild(UI.callout('tip',
      '둘 다 1차원으로 펴 줘요. 차이는 딱 하나예요 — <b>ravel 은 가능하면 뷰</b>, ' +
      '<b>flatten 은 항상 사본</b>. 원본을 지키고 싶으면 flatten, ' +
      '큰 배열을 복사 없이 훑고 싶으면 ravel 이에요. ' +
      '(원본이 연속이 아니면 ravel 도 어쩔 수 없이 사본을 만들어요.)'));
    return box;
  }
  ...
  Lab.register({
    ...
    n: '4',
    title: '배열의 모양 바꾸기와 뷰',
    blurb: 'size 가 같으면 모양은 자유롭게 바꿀 수 있어요. 그런데 모양이 바뀐 그 배열은 새 배열이 아니라 같은 메모리를 다른 창으로 보는 "뷰"인 경우가 많아요.',
    ...
    render: function (root) {
      ...
      root.appendChild(el('p', { class: 'lede', html:
        'NumPy 배열은 <b>값이 든 메모리 한 줄</b>과 <b>그것을 어떻게 읽을지 적어 둔 쪽지</b>로 되어 있어요. ' +
        'reshape 는 값을 옮기지 않고 그 쪽지만 고쳐 써요. 그래서 빠르고, 그래서 위험해요.' }));
      ...
      root.appendChild(el('p', { html:
        'reshape 가 허락하는 조건은 <b>size 가 같은 것</b> 하나뿐이에요. ' +
        'size ' + matrix.size + ' 인 이 배열은 ' + ND.shapeStr([8]) + ', ' +
        ND.shapeStr([4, 2]) + ', ' + ND.shapeStr([2, 2, 2]) +
        ' 로 자유롭게 바뀔 수 있어요. 값의 개수가 달라지는 모양만 거절해요.' }));

      root.appendChild(reshapeSim());

      root.appendChild(UI.callout('trap',
        '<code>-1</code> 은 "이 축의 크기는 네가 계산하세요" 라는 뜻이에요. ' +
        '<b>한 번만</b> 쓸 수 있고(두 번 쓰면 답이 하나로 정해지지 않아 에러), ' +
        'size 가 나누어떨어지지 않으면 -1 을 써도 에러예요. ' +
        '수업 노트북 셀 40 의 <code>matrix.reshape(-1, 3)</code> 이 바로 그 경우예요 — ' +
        'size 8 은 3 으로 나누어떨어지지 않아요. 위 시뮬레이터의 칩으로 직접 확인해 보세요.'));
      ...
      root.appendChild(el('p', { html:
        '몇 차원이든 1차원으로 펴는 일은 자주 필요해요. ' +
        '<code>flatten()</code> 과 <code>ravel()</code> 이 그 일을 하는데, ' +
        '결과 값은 똑같고 <b>원본과의 관계</b>가 달라요.' }));
      ...
      root.appendChild(el('p', { html:
        '배열 하나는 네 가지 정보로 되어 있어요: <b>메모리 한 줄(buffer)</b>, ' +
        '<b>shape</b>, <b>strides</b>, <b>offset</b>. ' +
        '뒤의 세 가지는 "그 메모리를 어떻게 읽을지" 적어 둔 쪽지일 뿐이에요. ' +
        '<b>뷰란 같은 메모리를 다른 쪽지로 보는 창</b>이에요.' }));
      ...
      (function () {
        ...
        root.appendChild(UI.ascii(
          ...
          '  a[0:2, 1:3] = shape ' + ND.shapeStr(v.shape) + '  strides ' + strTuple(v.strides) +
            '  offset ' + v.offset + '   → 같은 메모리에서 4칸만 골라 읽어요'));
      })();
      ...
      root.appendChild(el('p', { class: 'small muted', html:
        ...
        '<code>np.shares_memory</code> 로 확인한 결과예요. ' +
        '외우는 대신 이렇게 기억하면 돼요 — <b>"자리를 골라서 보는 것"은 뷰, ' +
        '"조건에 맞는 것만 모으는 것"은 사본.</b> ' +
        '슬라이싱은 규칙적인 간격이라 strides 로 표현할 수 있지만, ' +
        '불리언·팬시 인덱싱이 뽑아 오는 칸들은 간격이 불규칙해서 ' +
        'strides 하나로 표현할 방법이 없어요. 그래서 복사밖에 없어요.' }));

      root.appendChild(UI.callout('trap',
        '이것이 <b>초보자를 가장 많이 괴롭히는 버그의 원인</b>이에요. ' +
        '<code>부분 = data[0:10]</code> 로 잘라 놓고 <code>부분[0] = 0</code> 을 하면 ' +
        '원본 <code>data</code> 가 조용히 망가져요. 에러도 경고도 없어요. ' +
        '원본을 지켜야 한다면 <code>data[0:10].copy()</code> 라고 <b>명시적으로</b> 적어 보세요. ' +
        '거꾸로, 큰 배열을 다룰 때 습관적으로 <code>.copy()</code> 를 붙이면 ' +
        '메모리를 몇 배로 쓰게 돼요. 어느 쪽이 필요한지 생각하고 골라야 해요.'));

      root.appendChild(el('h3', { class: 'h-sub', text: 'strides — 뷰의 정체' }));
      root.appendChild(el('p', { html:
        '<code>a.strides</code> 는 <b>그 축으로 한 칸 옮길 때 메모리에서 몇 칸을 뛰어야 하는가</b>예요. ' +
        '(3, 4) 배열은 행을 하나 내려가면 다음 행의 시작까지 4칸을 뛰어야 하고, ' +
        '열을 하나 옮기면 바로 옆 칸이니 1칸이에요. 그래서 <b>(4, 1)</b> 이에요(원소 단위). ' +
        'NumPy 는 이것을 바이트로 출력하므로 int64(8바이트) 배열이면 <b>(32, 8)</b> 로 보여요.' }));

      root.appendChild(stridesSim());

      root.appendChild(UI.callout('why',
        '<code>a.T</code> 는 값을 하나도 옮기지 않아요. strides 를 뒤집을 뿐이에요. ' +
        '(4, 1) 이 (1, 4) 가 되면 "행 방향으로 1칸, 열 방향으로 4칸" 이 되어 ' +
        '같은 메모리가 전치된 행렬로 읽혀요. ' +
        '그래서 전치는 배열이 아무리 커도 시간이 거의 들지 않아요. ' +
        '대신 메모리를 건너뛰며 읽게 되므로 <b>연속(contiguous)이 아니게</b> 되고, ' +
        '그런 배열을 reshape 하면 NumPy 는 어쩔 수 없이 사본을 만들어요.'));

      root.appendChild(UI.callout('tip',
        '뷰를 <b>보장</b>받고 싶으면 shape 에 직접 대입하는 방법이 있어요 — ' +
        '<code>a.shape = (2, 6)</code>. 이 방법은 사본이 필요한 상황이면 조용히 복사하는 대신 ' +
        '에러를 내요. "복사가 일어나면 안 되는" 코드에서 안전장치로 쓸 수 있어요.'));

      /* ============================================ 4.4 */
      root.appendChild(el('h2', { class: 'h-sec', text: '4.4 차원 늘리기·줄이기' }));
      root.appendChild(el('p', { html:
        '값도 size 도 그대로 두고 <b>축의 개수만</b> 바꾸는 일이 자주 필요해요. ' +
        '<code>np.newaxis</code>(= <code>None</code>) 를 인덱스 자리에 끼워 넣으면 ' +
        '그 자리에 크기 1 인 축이 생겨요. <code>reshape</code> 로도, ' +
        '<code>np.expand_dims</code> 로도 같은 일을 할 수 있어요.' }));
      ...
      root.appendChild(UI.callout('tip',
        ...
        '<code>a[:, np.newaxis] + b</code> 처럼 한쪽을 세로로 세워야 해요. ' +
        '<code>(4,) + (3,)</code> 는 에러지만 <code>(4, 1) + (3,)</code> 은 (4, 3) 이 돼요. ' +
        '이 장에서 배운 축 끼워 넣기가 거기서 결정적으로 쓰여요.'));
      ...
      root.appendChild(UI.quiz([
        {
          ...
          choices: [
            'shape (2, 3) 배열이 되고 남는 두 칸은 0 으로 채워져요',
            'shape (3, 3) 배열이 되고 마지막 칸은 버려져요',
            '<code>ValueError</code> — size 8 을 3 으로 나눌 수 없어요',
            '-1 이 있으므로 언제나 성공해요'
          ],
          answer: 2,
          explain: 'reshape 는 원소를 <b>버리지도 만들지도 않아요.</b> ' +
            'size 8 을 3 으로 나누면 정수가 아니므로 -1 에 넣을 수가 없어요. ' +
            '수업 노트북 셀 40 에 일부러 남겨 둔 에러가 이것이에요. ' +
            '<code>reshape(-1, 2)</code> 나 <code>reshape(-1, 4)</code> 는 성공해요.'
        },
        {
          ...
          choices: ['4', '8', '16', '2'],
          answer: 2,
          explain: 'shape 는 (2, 2, 4) 이고 ndim 은 3 이에요. ' +
            'flatten 의 길이는 size, 즉 2 × 2 × 4 = <b>16</b> 이에요. ' +
            '8 을 고르는 것은 안쪽 두 축(2 × 4)만 세고 층이 2개인 것을 빠뜨린 실수예요. ' +
            '겉보기 줄 수가 아니라 <b>모든 축의 곱</b>을 세야 해요.'
        },
        {
          q: '<code>a = np.arange(12).reshape(3, 4)</code> 에서 ' +
             '<code>r = a.ravel()</code> 로 만든 뒤 <code>r[0] = 999</code> 를 실행했어요. ' +
             '<code>a[0, 0]</code> 은?',
          choices: [
            '0 — ravel 은 새 배열을 만들므로 원본은 그대로예요',
            '999 — a 가 연속이므로 ravel 은 뷰를 돌려주고, 같은 메모리를 고친 것이에요',
            '에러가 나요 — 뷰는 값을 바꿀 수 없어요',
            '999 지만 <code>a.flatten()</code> 을 썼어도 결과는 같아요'
          ],
          answer: 1,
          explain: '<code>ravel()</code> 은 <b>가능하면 뷰</b>예요. a 는 연속이므로 뷰가 되고, ' +
            'r 을 고친 것은 a 의 메모리를 고친 것이에요 → <code>a[0, 0]</code> 은 <b>999</b>. ' +
            '<code>flatten()</code> 은 <b>항상 사본</b>이라 이 경우 a 는 0 그대로예요. ' +
            '이 차이를 모르면 원본이 조용히 망가져요.'
        },
        {
          q: '<code>a = np.arange(1, 7).reshape(2, 3)</code> 에 대해 옳은 것은?',
          choices: [
            '<code>a.T</code> 와 <code>a.reshape(3, 2)</code> 는 shape 도 값도 같아요',
            'shape 는 둘 다 (3, 2) 지만 값의 배치가 달라요. <code>a.T</code> 는 strides 를 뒤집은 뷰예요',
            '<code>a.T</code> 는 값을 복사해 새 배열을 만들고 <code>reshape</code> 는 뷰예요',
            '<code>a[a > 3]</code> 은 뷰라서 고치면 a 도 바뀌어요'
          ],
          answer: 1,
          explain: 'shape 가 같다고 같은 배열이 아니에요. <code>a.T</code> 는 ' +
            '<code>[[1,4],[2,5],[3,6]]</code>, <code>a.reshape(3,2)</code> 는 ' +
            '<code>[[1,2],[3,4],[5,6]]</code> 이에요. ' +
            '전치는 strides 를 뒤집기만 하는 <b>뷰</b>이고 값을 옮기지 않아요. ' +
            '한편 <code>a[a > 3]</code> 같은 불리언 인덱싱은 뽑는 칸의 간격이 불규칙해서 ' +
            'strides 로 표현할 수 없으므로 <b>항상 사본</b>이에요.'
        }
      ], { id: 'reshape' }));
    }
  });
