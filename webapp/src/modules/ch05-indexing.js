...
(function () {
  ...
  var DESC2 = {
    '0': '0행 <b>전체</b>. 정수 하나 → 축 0 이 사라져 1차원이 돼요.',
    '0:1': '똑같이 0행이지만 <b>슬라이스</b>라서 축 0 이 살아남아요. shape (1, 8).',
    ':, 0': '0열 전체. 파이썬 list 로는 <code>[row[0] for row in lst]</code> 를 써야 하는 일이에요.',
    ':, 0:1': '같은 0열인데 2차원을 유지해요. shape (6, 1).',
    '2, 3': '정수 2개 → 축이 둘 다 사라져요. 0차원(스칼라)이에요.',
    '2:3, 3:4': '같은 칸 하나인데 슬라이스 2개라서 shape (1, 1) 2차원이에요.',
    '1:4': '행 1·2·3. <b>stop 인 4 는 포함되지 않아요.</b>',
    '1:4, ::2': '행 1~3, 열은 0부터 2칸씩. 행과 열을 따로 자를 수 있는 것이 list 와 다른 점이에요.',
    '-1': '마지막 행. 음수 인덱스는 뒤에서부터 세요.',
    '-2:': '뒤에서 두 행. stop 을 생략하면 끝까지예요.',
    '::-1': 'step 이 −1 → 행 순서가 뒤집혀요. 값을 복사하지 않고 <b>보폭만 −8 로 바꾼</b> 뷰예요.',
    ':, ::-1': '열 순서를 뒤집어요.',
    '::2, ::2': '행·열 모두 한 칸씩 건너뛰어요. 체스판처럼 성기게 뽑혀요.',
    '::-2': '뒤에서부터 두 칸씩.',
    '9': '<b>정수</b> 인덱스가 범위를 넘었어요 → 바로 IndexError.',
    '9:20': '같은 범위인데 <b>슬라이스</b>라서 에러가 아니에요. 있는 만큼(없으므로 0개) 줘요.',
    '4:20': '슬라이스는 넘친 부분을 조용히 잘라 내요. 4·5행만 나와요.',
    '2:2': 'start 와 stop 이 같으면 빈 배열이에요.',
    '0, 0, 0': '2차원인데 인덱스를 3개 줬어요 → IndexError.',
    '...': 'Ellipsis 하나는 "남은 축 전부"라서 <code>a[:, :]</code> 와 같아요.',
    '..., 1': '마지막 축의 1 번. 축이 몇 개든 "맨 뒤 축"을 가리킬 때 써요. 여기서는 1열.',
    ':, None': 'None(=np.newaxis) 은 길이 1 인 축을 <b>끼워 넣어요</b>. (6, 8) → (6, 1, 8).',
    'None': '맨 앞에 축을 하나 추가해요. (6, 8) → (1, 6, 8).',
    '-1, -1': '오른쪽 아래 끝 칸.'
  };

  var DESC3 = {
    '0': '0번 <b>장</b> 전체. mnist 로 치면 첫 번째 이미지 한 장이에요.',
    '1': '1번 장 전체.',
    '0:1': '같은 0번 장이지만 슬라이스라서 3차원을 유지해요. shape (1, 3, 4).',
    '::-1': '장 순서를 뒤집어요.',
    '1, 2, 3': '(장 1, 행 2, 열 3) 원소 하나. 정수 3개 → 0차원.',
    '0, 2': '0번 장의 2번 행. 뒤쪽 축(열)은 생략했으니 전체예요.',
    ':, :, 0': '모든 장의 0번 열. 세로 한 줄씩 모은 것.',
    ':, 1, :': '모든 장의 1번 행.',
    '0, :, ::2': '0번 장에서 열을 한 칸씩 건너뛰어요.',
    '..., 1': '맨 뒤 축의 1 번 → 모든 장·행의 1번 열. <code>[:, :, 1]</code> 과 같아요.',
    '..., 0:2': '맨 뒤 축을 0~1 만. <code>[:, :, 0:2]</code> 와 같아요.',
    '2': '장이 2개(0, 1)뿐인데 2 를 요청했어요 → IndexError.',
  ...
  var GROUPS2 = [
    ...
    { t: '범위를 넘기면 — 슬라이스는 봐 주고 정수는 화내요', items: ['9', '9:20', '4:20', '2:2', '0, 0, 0'] },
  ...
  function hintFor(m) {
    if (/out of bounds/.test(m)) {
      return '<b>정수</b> 인덱싱은 범위를 넘으면 그 자리에서 IndexError 예요. ' +
        '같은 자리를 <code>9:20</code> 처럼 <b>슬라이스</b>로 바꿔 보세요 — 에러 없이 빈 배열이 나와요.';
    }
  ...
  function simPlayground() {
    ...
    var dimSeg = UI.seg({
      ...
      onChange: function (v) {
        st.dim = v; buildChips();
        input.setValue(v === '2' ? '1:4, ::2' : '0');   // setValue 가 onChange → draw 까지 불러요
      }
    });
    ...
    function draw() {
      ...
      try {
        ...
        host.appendChild(el('div', { class: 'flow' }, [
          ...
          opArrow('✗'),
          panel('결과', null, el('div', { class: 'muted small', text: '없어요 — 예외가 났어요.' }))
        ]));
      ...
      var right;
      if (res.ndim > 3) {
        right = UI.out(ND.format(res), { label: '4차원 이상은 격자로 못 그려요' });
      } else if (res.size === 0) {
        right = el('div', { class: 'muted small', style: { padding: '.6rem 0' },
          text: '빈 배열이에요 — 칠할 칸이 없어요. 그래도 에러는 아니에요.' });
      } else {
      ...
      if (base.ndim - nInt + nNew === res.ndim) {
        host.appendChild(line('<span class="mono">ndim: ' + base.ndim + ' − 정수 ' + nInt +
          ...
          '<span class="muted">정수는 축을 없애고, 슬라이스는 축을 남겨요.</span>'));
      }
      ...
      host.appendChild(el('p', { class: 'small', style: { color: 'var(--s3)' }, html:
        '<b>np.shares_memory(a, a[' + UI.esc(shown) + ']) → True</b> — 기본 인덱싱 결과는 <b>뷰(view)</b>예요. ' +
        '값을 복사하지 않고 시작 위치(offset)와 보폭(strides)만 바꿔 같은 메모리를 다르게 봐요.' }));
    ...
    return UI.card({
      kicker: '시뮬레이터',
      title: '슬라이싱 플레이그라운드',
      note: '입력창에 인덱스식을 쓰면 <b>왼쪽 원본에서 뽑히는 칸</b>과 <b>결과 배열</b>이 바로 바뀌어요. ' +
        '<code>a[1:4, ::2]</code> 처럼 껍질을 씌워 써도 돼요. 아래 칩을 눌러 가며 shape 가 어떻게 변하는지 눈으로 따라가 봐요.',
  ...
  function simRuler() {
    ...
    function draw() {
      ...
      host.appendChild(note(st.useStop === 'v'
        ? '<b>stop 인 ' + st.stop + ' 은 포함되지 않아요.</b> 마지막으로 뽑히는 인덱스는 ' +
          (res.size ? String(res.get([res.size - 1])) + ' 예요.' : '없어요.')
        : 'stop 을 생략하면 ' + (st.step > 0 ? '끝까지' : '맨 앞까지') + ' 가요.'));
    }
    ...
    return UI.card({
      ...
      note: '<code>stop</code> 이 포함되지 않는다는 것과, 음수 <code>step</code> 이 방향을 뒤집는다는 것을 ' +
        '슬라이더로 확인해 봐요. <code>start</code>·<code>stop</code> 을 생략하면 어떤 값이 대신 들어가는지도 아래에 적혀요.',
      body: [ctl, host]
    });
  }
  ...
  function simMask() {
    ...
    function draw() {
      ...
      host.appendChild(el('div', { class: 'flow' }, [
        ...
        panel(t.name + '[cond] — ' + ND.shapeStr(picked.shape), 'r',
          picked.size ? arrGrid(picked, all('r'))
            : el('div', { class: 'muted small', text: '조건을 만족하는 칸이 없어요 → 빈 배열' }))
      ]));
      ...
      host.appendChild(note(a.ndim >= 2
        ? '원본이 ' + a.ndim + '차원인데 결과는 <b>1차원</b>이에요. 조건을 만족하는 칸이 격자 모양으로 모여 있을 이유가 없으니, ' +
          'NumPy 는 골라낸 값을 한 줄로 늘어놓아요.'
        : '<code>cond</code> 는 원본과 <b>shape 가 같은</b> 불리언 배열이에요. True 인 자리의 값만 순서대로 뽑혀요.'));
    ...
    return UI.card({
      ...
      note: '연산자와 임계값을 움직이면 <b>불리언 배열(마스크)</b>과 걸러낸 결과가 함께 바뀌어요. ' +
        '조건을 두 개 조합할 때 <code>&amp;</code>(그리고)·<code>|</code>(또는)·<code>~</code>(부정)를 쓴다는 것도 여기서 확인해요.',
      body: [ctlHost, host]
    });
  }
  ...
  function simFancy() {
    ...
    function buildCtl() {
      ...
      items.push(el('div', { style: { marginTop: '.6rem' } }, [
        ...
        }) : note('인덱스 개수가 짝수일 때 2차원으로 바꿔 볼 수 있어요.')
      ]));
    ...
    function draw() {
      ...
      if (!st.idx.length) {
        host.appendChild(note('인덱스 배열이 비었어요. 위의 칩으로 값을 넣어 봐요.'));
        return;
      }
      ...
      if (!st.two) {
        ...
        host.appendChild(note('인덱스 칸에 마우스를 올리면 그 값이 원본의 어느 칸을 가리키는지 진하게 표시돼요.'));
      } else {
        host.appendChild(el('div', { class: 'flow' }, [
      ...
      if (err) {
        ...
        host.appendChild(note('팬시 인덱싱도 <b>정수 인덱싱</b>이에요. 범위를 넘으면 슬라이싱처럼 봐 주지 않고 IndexError 를 내요. ' +
          '음수는 뒤에서부터 세므로 <code>-1</code> 은 정상이에요.'));
        return;
      }
      ...
      host.appendChild(line('원본은 <b>' + src.size + '칸</b>인데 결과는 <b>' + res.size + '칸</b>이에요. ' +
        '결과의 shape 는 원본이 아니라 <b>인덱스 배열의 shape</b>(' + ND.shapeStr(idxArr.shape) + ')를 그대로 따라요.' +
        (res.size > src.size ? ' 같은 칸을 여러 번 가리켜도 되니 <b>원본보다 길어질 수 있어요.</b>' : '')));
      host.appendChild(el('p', { class: 'small', style: { color: 'var(--s3)' }, html:
        '<b>np.shares_memory(temp5, temp5[temp6]) → False</b> — 팬시 인덱싱 결과는 <b>사본</b>이에요. ' +
        '뽑는 순서가 규칙적이지 않으니 offset·strides 로는 표현할 수 없고, 새 메모리에 값을 복사해야 해요.' }));
    }
    ...
    return UI.card({
      kicker: '시뮬레이터',
      title: '팬시 인덱싱 조립기',
      note: '인덱스 배열 <code>temp6</code> 를 직접 만들어 봐요. 같은 인덱스를 여러 번 넣어도 되고, ' +
        '원본보다 길게 만들어도 돼요. 범위를 벗어나면 어떻게 되는지도 눌러 보세요.',
      body: [ctlHost, host]
    });
  }
  ...
  function part1D(root) {
    sec(root, '5.1  1차원 인덱싱 — list 와 똑같아요');
    p(root, '먼저 안심할 것. 1차원 배열의 인덱싱은 파이썬 list 와 <b>완전히 같아요.</b> ' +
      '0 부터 세고, 음수는 뒤에서부터 세요. 새로 외울 것이 없어요.');
      ...
      if (ok) {
        dialHost.appendChild(line(stI.i < 0
          ? '음수 인덱스 <span class="mono">' + stI.i + '</span> 는 <span class="mono">' + stI.i + ' + 6 = ' + norm +
            '</span> 로 바뀌어요. <span class="mono">temp4[' + stI.i + '] = temp4[' + norm + ']</span>'
          : '<span class="mono">temp4[' + stI.i + ']</span> 는 앞에서부터 ' + (stI.i + 1) + '번째 값이에요.'));
      } else {
        dialHost.appendChild(UI.errBlock(msg));
        dialHost.appendChild(note('원소가 6개면 쓸 수 있는 정수 인덱스는 0~5 와 −6~−1 뿐이에요.'));
      }
    }
    ...
    root.appendChild(UI.callout('tip',
      '여기까지는 list 와 판박이에요. 진짜 차이는 <b>차원이 둘 이상</b>이 되는 순간부터 나타나요.'));
  }
  ...
  function partND(root) {
    ...
    p(root, '2차원 배열에서 NumPy 는 <b>콤마 하나로</b> 두 축을 동시에 지정해요. ' +
      '<code>a[i, j]</code> 예요. list 처럼 <code>a[i][j]</code> 라고 써도 같은 값이 나오지만, ' +
      '둘은 속으로 하는 일이 달라요.');
    ...
    root.appendChild(note('오른쪽은 먼저 0행 전체(주황)를 <b>임시 뷰</b>로 만들고, 그 뷰에서 다시 1번을 꺼내요.'));

    root.appendChild(UI.callout('why',
      '<code>insl[0][1]</code> 은 <b>인덱싱을 두 번</b> 해요. 첫 번째 <code>insl[0]</code> 이 길이 3 인 ' +
      '뷰 객체를 하나 만들고, 두 번째 <code>[1]</code> 이 거기서 값을 꺼내요. ' +
      '<code>insl[0, 1]</code> 은 두 축의 좌표를 한꺼번에 받아 <b>메모리 위치를 한 번만 계산</b>해요. ' +
      '결과는 같지만 중간 객체가 없으니 더 빠르고, 무엇보다 <code>a[:, 0]</code> 처럼 ' +
      '<b>list 로는 못 쓰는 표기</b>가 가능해져요.'));

    sub(root, '중간에 생긴 뷰가 원본을 바꿔요 (셀 49)');
    p(root, '수업 노트북 셀 49 는 <code>insl[0][0] = 10</code> 을 실행한 뒤 원본을 출력해요. ' +
      '원본이 바뀌어요. <code>insl[0]</code> 이 <b>사본이 아니라 뷰</b>이기 때문이에요.');
    ...
    root.appendChild(note('메모리 막대에서 <code>insl[0]</code> 이 차지한 칸은 <code>insl</code> 의 칸과 완전히 겹쳐요. ' +
      '따로 복사한 것이 아니라 <b>같은 메모리를 다르게 보는 것</b>이에요. 4장에서 본 그 이야기예요.'));

    sub(root, '열 하나 뽑기 — list 로는 반복문이 필요해요');
    root.appendChild(UI.code(
      'lst = [[1, 2, 3], [4, 5, 6]]\n' +
      '[row[0] for row in lst]     # list: 반복문을 돌려야 한다\n\n' +
      'insl[:, 0]                  # NumPy: 표기 하나로 끝나요'));
    root.appendChild(el('div', { class: 'flow' }, [
    ...
    p(root, '행은 <code>lst[0]</code> 로 쉽게 꺼내지만 열은 그렇지 않아요. ' +
      '2차원 배열을 다루는 일의 절반은 "열 하나 뽑기" 인데, ' +
      '이것을 표기 하나로 할 수 있다는 것이 NumPy 를 쓰는 큰 이유 중 하나예요.');

    sub(root, '3차원 — (장, 행, 열)');
    p(root, 'mnist 이미지 데이터셋은 <code>(장, 행, 열)</code> 3차원이에요. ' +
      '<code>train_imgs[6, 10, 12]</code> 는 "7번째 이미지의 10행 12열 픽셀"이에요. ' +
      '축이 늘어도 규칙은 그대로예요 — 콤마로 축의 좌표를 순서대로 적어요.');
  ...
  function partSlice(root) {
    ...
    p(root, '슬라이싱은 <code>start:stop:step</code> 세 값으로 "어디서부터, 어디 앞까지, 몇 칸씩" 을 적어요. ' +
      '가장 많이 틀리는 것은 <b>stop 이 포함되지 않아요</b>는 점이에요. ' +
      '<code>a[1:4]</code> 는 1, 2, 3 이고 4 는 빠져요.');
    ...
    root.appendChild(UI.table(
      ...
      [
        ...
        { w: 'step 음수', m: '거꾸로 훑어요', e: '<code>a[::-1]</code>' },
    ...
    p(root, '셀 49 에서 <code>insl[0][0] = 10</code> 을 이미 실행했으므로, ' +
      '이 시점의 <code>insl</code> 은 <span class="mono">[[10 2 3] [4 5 6]]</span> 이에요. ' +
      '노트북을 위에서부터 순서대로 실행했다면 값이 이렇게 나와요.');
    ...
    root.appendChild(UI.callout('why',
      '셀 56 의 <code>insl[1:3]</code> 은 행이 2개뿐인 배열에 "1행부터 2행까지" 를 요청해요. ' +
      '2행은 없어요. 그런데 <b>에러가 아니에요</b> — NumPy 는 있는 만큼인 1행 하나만 줘요. ' +
      '슬라이스는 "요청한 범위와 실제 범위의 교집합"을 조용히 계산해요. 파이썬 list 슬라이싱도 같은 규칙이에요.'));

    sub(root, '결정적 대비 — 슬라이스는 봐 주고, 정수는 화내요');
    ...
    root.appendChild(UI.table(
      ...
      [
        tryRow('1:3', '슬라이스 — 있는 만큼(1행) 줘요'),
        tryRow('5:9', '슬라이스 — 겹치는 것이 없으면 빈 배열. 에러가 아니에요'),
        tryRow('2', '정수 — 2행은 없어요. 값을 만들어 낼 수 없으니 IndexError'),
        tryRow('-3', '정수 — −3 + 2 = −1 로 여전히 범위 밖이에요'),
        tryRow('0, 5', '정수 — 열 축에서도 마찬가져요')
      ]));
    root.appendChild(UI.callout('why',
      '왜 이렇게 다른가. 슬라이스는 <b>배열을 돌려주는</b> 요청이에요. 요청한 칸이 없으면 원소가 없는 배열을 주면 되니 ' +
      '돌려줄 것이 있어요. 정수 인덱싱은 <b>값 하나(또는 한 축 낮은 배열)를 돌려주는</b> 요청이에요. ' +
      '없는 값은 만들어 낼 수 없으므로 에러밖에 답이 없어요. ' +
      '플레이그라운드에서 <code>9</code> 와 <code>9:20</code> 을 번갈아 눌러 이 차이를 직접 확인하세요.'));

    sub(root, '정수는 축을 없애고, 슬라이스는 축을 남겨요');
    p(root, '이것이 5장에서 가장 중요한 규칙이에요. 값이 같아도 <b>차원이 다르면 다른 배열</b>이고, ' +
      '뒤따르는 연산에서 브로드캐스팅 결과가 달라져요.');
    ...
    root.appendChild(UI.table(
      ...
      [
        ['0', '정수 1개 → 축 0 이 사라져요'],
        ['0:1', '슬라이스 → 축 0 이 길이 1 로 남아요'],
        [':, 0', '정수 1개 → 축 1 이 사라져요'],
        [':, 0:1', '슬라이스 → 축 1 이 길이 1 로 남아요'],
        ['2, 3', '정수 2개 → 축이 둘 다 사라져요 (스칼라)'],
        ['2:3, 3:4', '슬라이스 2개 → 둘 다 남아요']
      ].map(function (r) {
        var v = a68.idx(r[0]);
    ...
    root.appendChild(note('값은 똑같이 6개예요. 그런데 왼쪽은 가로 한 줄(1차원), 오른쪽은 세로 한 열(2차원)이에요.'));

    var RAT = (D && D.nd) ? D.nd('ratingsSample') : null;
    if (RAT) {
      root.appendChild(UI.callout('tip',
        '3장에서 본 <code>data[:, :1]</code> 이 바로 이 규칙을 쓴 것이에요. 이 페이지에 든 평점 표본은 ' +
        '<span class="mono">' + ND.shapeStr(RAT.shape) + '</span> 인데(실제 전체는 ' +
        '<span class="mono">' + ND.shapeStr(D.ratingsMeta.trueShape) + '</span> 이고, ' +
        '여기 실린 것은 재배포 제한 때문에 구조만 같게 만든 합성 데이터예요), ' +
        ...
        ND.shapeStr(RAT.idx(':, 0').shape) + '</span> 로 <b>1차원 목록</b>이 돼요. ' +
        '표를 계속 표로 다루고 싶으면 슬라이스를, 값들만 필요하면 정수를 써요.'));
    }

    sub(root, '... (Ellipsis)');
    p(root, '<code>...</code> 은 "여기 남은 축은 전부" 라는 뜻이에요. ' +
      '축이 몇 개인지 모르는 배열에서 <b>맨 뒤 축</b>만 지정하고 싶을 때 써요.');
    root.appendChild(UI.code(
    ...
    root.appendChild(note('두 식의 shape 는 모두 <span class="mono">' +
      ND.shapeStr(a3.idx('..., 1').shape) + '</span> 예요. ' +
      '<code>...</code> 은 딱 한 번만 쓸 수 있어요.'));

    sub(root, '슬라이스 결과는 뷰예요 — 대입하면 원본이 바뀌어요');
    p(root, '여기서 파이썬 list 와 NumPy 가 <b>정반대</b>로 갈려요. ' +
      'list 슬라이싱은 <b>사본</b>을 만들고, NumPy 슬라이싱은 <b>뷰</b>를 만들어요. ' +
      '같은 코드 모양인데 결과가 달라요.');
    ...
    root.appendChild(UI.callout('trap',
      'list 를 쓰던 습관으로 <code>backup = data[:]</code> 라고 쓰면 백업이 되지 않아요. ' +
      '뷰라서 원본과 같은 메모리를 가리키고, 원본을 고치면 "백업" 도 같이 바뀌어요. ' +
      '진짜 사본이 필요하면 <code>data.copy()</code> 를 쓰세요.'));

    sub(root, '슬라이스에 통째로 대입하기');
    p(root, '뷰라는 성질은 불편한 함정이기만 한 것이 아니에요. ' +
      '<b>배열의 일부를 한 번에 고치는</b> 가장 좋은 도구가 돼요.');
    var stZ = { done: false };
    var zHost = el('div');
    function zeroDemo() {
      UI.clear(zHost);
    ...
    root.appendChild(note('오른쪽 변에 배열을 놓아도 돼요. ' +
      '<code>a[0, :] = [9, 9, 9, 9]</code> 나 <code>a[:, 0] = a[:, 1]</code> 처럼 쓸 수 있고, ' +
      '길이가 안 맞으면 브로드캐스팅 규칙을 따라요(7장).'));
  }
  ...
  function partBool(root) {
    ...
    p(root, '배열을 숫자와 비교하면 값이 아니라 <b>참·거짓의 배열</b>이 나와요. ' +
      '이 불리언 배열을 그대로 인덱스로 넣으면 True 인 자리의 값만 뽑혀요. ' +
      '반복문도 <code>if</code> 문도 쓰지 않아요.');
    ...
    sub(root, 'True 는 1, False 는 0 이에요');
    p(root, '불리언 배열의 <code>dtype</code> 은 <code>bool</code> 이고, 계산할 때는 True=1, False=0 으로 쓰여요. ' +
      '그래서 <code>.sum()</code> 은 <b>조건을 만족하는 개수</b>, <code>.mean()</code> 은 <b>그 비율</b>이 돼요. ' +
      '이 두 줄로 "몇 개인가 / 몇 퍼센트인가" 를 바로 답할 수 있어요.');
    root.appendChild(UI.code(
      'cond = temp4 > 35\n' +
      'cond.dtype       # bool\n' +
      'cond.sum()       # 참인 개수\n' +
    ...
    p(root, '조건을 조합할 때 파이썬의 <code>and</code>·<code>or</code>·<code>not</code> 은 ' +
      '<b>쓸 수 없어요.</b> 대신 <code>&amp;</code>, <code>|</code>, <code>~</code> 를 쓰고, ' +
      '<b>각 조건을 반드시 괄호로 묶어요.</b>');
    ...
    root.appendChild(UI.callout('why',
      '파이썬의 <code>and</code> 는 왼쪽 값을 먼저 <b>하나의 참/거짓</b>으로 판단해요. ' +
      '그런데 <code>temp4 &gt; 15</code> 는 원소가 6개인 배열이에요 — 이걸 참 하나로 정하려면 ' +
      '"하나라도 참이면 참" 인지 "모두 참이어야 참" 인지 정해야 하는데, NumPy 는 ' +
      '<b>멋대로 정하지 않고 거부해요.</b> 그래서 저 메시지가 <code>any()</code> 와 <code>all()</code> 을 알려 줘요. ' +
      '반면 <code>&amp;</code> 는 원소별로 계산하므로 배열 두 개를 자리마다 짝지어 처리해요 — 우리가 원하는 것이에요.'));

    root.appendChild(UI.callout('trap',
      '괄호를 빼면 이런 일이 나요. <code>temp4 &gt; 15 &amp; temp4 &lt; 45</code> 에서 ' +
      '<code>&amp;</code> 는 비교 연산자 <code>&gt;</code> 보다 <b>우선순위가 높아요.</b> ' +
      '그래서 파이썬은 <code>temp4 &gt; (15 &amp; temp4) &lt; 45</code> 로 읽고, ' +
      '연쇄 비교가 되면서 결국 위와 똑같은 ValueError 가 나요. ' +
      '<b>조건마다 괄호</b> — 예외 없는 규칙으로 외워 두는 것이 이득이에요.'));

    root.appendChild(UI.callout('ver',
      '수업 노트북 셀 147 은 <code>np.NaN</code>, <code>np.Inf</code> 를 써요. ' +
      '이 이름들은 <b>NumPy 2.0 에서 삭제되었어요</b> — 그대로 실행하면 ' +
      '<code>AttributeError: np.NaN was removed in the NumPy 2.0 release. Use np.nan instead.</code> 가 나요. ' +
      '소문자 <code>np.nan</code>, <code>np.inf</code> 를 써야 해요. ' +
      '사본 관련해서도 하나: <code>np.array(x, copy=False)</code> 는 2.0 부터 ' +
      '사본이 필요한 상황이면 에러를 내요. 사본을 피하고 싶으면 <code>np.asarray(x)</code> 를 써요.'));

    root.appendChild(UI.code(
      'temp3 = np.array([1, np.nan, np.inf])   # 2.x 에서는 소문자\n' +
      'np.isnan(temp3)\n' +
      'temp3[~np.isnan(temp3)]                # 결측치만 걸러내요'));
    ...
    root.appendChild(note('<code>~</code> 는 마스크를 뒤집어요. "결측치가 아닌 것만 남기기" 는 실전에서 가장 자주 쓰는 마스커요.'));

    sub(root, '2차원에서 — 행 단위로 고르려면 마스크가 1차원이어야 해요');
    p(root, '2차원 배열에 <b>같은 shape 의</b> 마스크를 넣으면 결과가 1차원으로 납작해져요. ' +
      '반면 <b>행 축과 길이가 같은 1차원</b> 마스크를 넣으면 "행 고르기" 가 되어 2차원이 유지돼요. ' +
      '마스크의 shape 가 무엇을 고를지 결정해요.');
    ...
    if (inf) {
      ...
      root.appendChild(UI.statRow([
        ...
        { k: 'data[hot].shape', v: ND.shapeStr(picked.shape), sub: '2차원이 유지돼요' }
      ]));
      root.appendChild(UI.callout('why',
        '<code>hot</code> 의 길이(' + hot.shape[0] + ')가 <code>data</code> 의 <b>행 개수</b>와 같으므로 ' +
        'NumPy 는 이것을 "행을 고르세요" 는 뜻으로 읽어요. 고른 행은 통째로 살아남으니 열 축(40)이 그대로 붙어 ' +
        '<span class="mono">' + ND.shapeStr(picked.shape) + '</span> 가 돼요. ' +
        '만약 <code>data > 6</code> 처럼 <span class="mono">(60, 40)</span> 마스크를 넣었다면 ' +
        '골라진 칸들이 흩어져 있으니 결과는 1차원 ' +
        '<span class="mono">' + ND.shapeStr(ND.maskSelect(inf, ND.ops.gt(inf, 6)).shape) + '</span> 가 돼요.'));
      ...
      root.appendChild(note('마스크 길이는 그 축의 길이와 정확히 같아야 해요. NumPy 는 부족한 만큼 채워 주지 않아요.'));
    }

    sub(root, '결과는 사본인데, 대입은 원본을 바꿔요');
    p(root, '이 절의 마지막 미묘한 지점이에요. <code>a[a > 5]</code> 로 <b>꺼낸 배열</b>은 사본이라 ' +
      '고쳐도 원본에 영향이 없어요. 그런데 <code>a[a > 5] = 0</code> 처럼 <b>대입문의 왼쪽</b>에 쓰면 ' +
      '원본이 바뀌어요. 꺼낼 때와 넣을 때가 다른 것이에요.');
    ...
    function boolDemo() {
      ...
      bHost.appendChild(el('p', { class: 'small', style: { color: 'var(--s3)' }, html:
        ...
        '불리언 인덱싱 결과는 새 메모리에 담긴 <b>사본</b>이에요.' }));
      bHost.appendChild(el('div', { class: 'stack-2' }, [
        ...
        el('div', null, [el('div', { class: 'panel-t r', text: 'b 의 메모리 (따로 있어요)' }),
    ...
    root.appendChild(UI.callout('why',
      '<code>b = a[a &gt; 5]</code> 는 값을 <b>읽어 오는</b> 식이에요. 뽑히는 자리가 규칙적이지 않아 ' +
      'offset·strides 로 표현할 수 없으니, NumPy 는 새 메모리를 잡아 값을 복사해요 → 사본. ' +
      '<code>a[a &gt; 5] = 0</code> 은 파이썬이 <code>a.__setitem__</code> 을 부르는 식이에요. ' +
      '중간 배열을 만들 필요 없이 <b>원본의 해당 칸에 바로 써요</b> → 원본이 바뀌어요.'));
  }
  ...
  function partFancy(root) {
    ...
    p(root, '인덱스 자리에 <b>정수 배열</b>을 넣을 수도 있어요. ' +
      '그 배열의 각 값이 원본의 몇 번 칸인지를 가리키고, 가리킨 순서대로 값이 나와요. ' +
      '순서를 바꾸거나, 같은 값을 여러 번 뽑거나, 원하는 몇 개만 골라올 때 써요.');
    ...
    sub(root, '2차원에서 — 블록이 아니에요');
    p(root, '여기서 거의 모두가 한 번은 틀려요. <code>a[[0, 2], [1, 3]]</code> 은 ' +
      '"0·2행 × 1·3열 블록" 이 아니에요. 두 리스트를 <b>자리마다 짝지어</b> ' +
      '<code>(0, 1)</code> 과 <code>(2, 3)</code> <b>두 원소</b>만 뽑아요.');
    ...
    root.appendChild(el('div', { class: 'flow' }, [
      ...
      opArrow('≠'),
      panel('많이들 기대하는 것 — 블록 4개 (틀렸어요)', null, UI.grid(a34, {
        axisLabels: true, cellSize: CS,
        highlight: function (i) {
    ...
    root.appendChild(UI.callout('why',
      '두 인덱스 배열이 <b>브로드캐스팅되어 좌표 짝</b>이 만들어진다고 생각하면 돼요. ' +
      '<code>[0, 2]</code> 와 <code>[1, 3]</code> 은 둘 다 길이 2 이므로 짝은 ' +
      '<code>(0,1)</code>, <code>(2,3)</code> 두 개뿐이에요. ' +
      '그래서 결과도 원소 2개인 1차원 배열이에요.'));

    p(root, '정말 블록을 원한다면 두 가지 방법이 있어요.');
    root.appendChild(UI.code(
      'a[np.ix_([0, 2], [1, 3])]     # 방법 1: 좌표 짝을 격자로 펼쳐 준다\n' +
      'a[[0, 2]][:, [1, 3]]          # 방법 2: 행 고른 뒤 열 고르기 (사본이 두 번 생겨요)'));
    root.appendChild(el('div', { class: 'flow' }, [
    ...
    root.appendChild(note('<code>np.ix_</code> 는 <code>[0, 2]</code> 를 ' +
      ...
      '바꿔 줘요. 브로드캐스팅되면 <span class="mono">(2, 2)</span> 좌표 격자가 되므로 블록이 나와요.'));

    sub(root, 'np.argsort — 정렬 순서를 인덱스로 받아 쓰기');
    p(root, '팬시 인덱싱이 가장 빛나는 곳이에요. <code>np.argsort</code> 는 값을 정렬하지 않고 ' +
      '<b>정렬했을 때의 순서</b>를 인덱스 배열로 줘요. 그 인덱스를 그대로 넣으면 정렬된 배열이 돼요.');
    ...
    root.appendChild(note('<code>order</code> 의 첫 값 ' + order.get([0]) +
      ' 은 "가장 작은 값은 ' + order.get([0]) + '번 칸에 있어요" 는 뜻이에요. ' +
      '값이 아니라 <b>위치</b>를 준다는 것이 요점이에요.'));
    ...
    if (inf) {
      ...
      root.appendChild(note('환자 번호와 그 값이 짝을 유지한 채 함께 따라와요. ' +
        '이것이 "정렬해서 상위 몇 개 보기" 를 반복문 없이 하는 표준 방법이에요.'));
    }
    ...
    root.appendChild(UI.table(
      ...
      [
        { t: '기본 (정수·슬라이스)', e: '<code>a[1:4, ::2]</code>',
          v: '<b style="color:var(--s3)">뷰</b>', s: '규칙적으로 줄어들어요 (정수는 축 삭제)',
        ...
        { t: '팬시 (정수 배열)', e: '<code>a[[0, 2, 2]]</code>',
          v: '<b>사본</b>', s: '인덱스 배열의 shape 를 따라요',
          u: '순서를 바꿀 때. 중복해서 뽑을 때. argsort 와 함께' }
      ]));
    root.appendChild(UI.callout('tip',
      '헷갈릴 때 판단 기준은 하나예요. <b>offset 과 strides 만으로 그 칸들을 훑을 수 있나요?</b> ' +
      '가능하면 뷰를 줄 수 있고(기본 인덱싱), 불가능하면 복사밖에 방법이 없어요(불리언·팬시). ' +
      '확실하지 않으면 <code>np.shares_memory(a, b)</code> 로 물어보면 돼요.'));
  }
  ...
  Lab.register({
    ...
    n: '5',
    title: '인덱싱과 슬라이싱',
    blurb: '배열에서 원하는 부분만 꺼내는 네 가지 방법과, 꺼낸 것이 원본과 메모리를 나눠 쓰는지를 직접 확인해요.',
    sim: '슬라이싱 플레이그라운드 · 슬라이스 눈금자 · 마스크 만들기 · 팬시 인덱싱 조립기',
    render: function (root) {
      root.appendChild(el('p', { class: 'lede', html:
        '배열을 만드는 것보다 <b>필요한 부분만 꺼내는 일</b>이 훨씬 자주 필요해요. ' +
        'NumPy 의 인덱싱은 파이썬 list 에서 출발하지만, 축이 둘 이상이 되는 순간부터 ' +
        '표기도 규칙도 달라져요. 이 장의 목표는 두 가지 규칙을 손에 익히는 것이에요 — ' +
        '<b>정수는 축을 없애고 슬라이스는 축을 남겨요</b>, 그리고 ' +
        '<b>기본 인덱싱은 뷰, 불리언·팬시는 사본이에요.</b>' }));

      root.appendChild(simPlayground());
      root.appendChild(note('이 플레이그라운드는 이 장 전체에서 계속 참조해요. ' +
        '새 규칙을 읽을 때마다 여기로 돌아와 직접 넣어 보는 것이 가장 빠른 길이에요.'));
      ...
      root.appendChild(UI.quiz([
        {
          q: '<code>a = np.arange(12).reshape(3, 4)</code> 일 때 ' +
             '<code>a[1]</code> 과 <code>a[1:2]</code> 의 shape 는 각각 무엇인가요?',
          choices: [
            '<code>(4,)</code> 와 <code>(1, 4)</code>',
            '둘 다 <code>(4,)</code> — 값이 같으니 shape 도 같아요',
            '둘 다 <code>(1, 4)</code>',
            '<code>(1, 4)</code> 와 <code>(4,)</code>'
          ],
          answer: 0,
          explain: '값은 둘 다 1행의 네 숫자지만 차원이 달라요. <b>정수 인덱스는 그 축을 없애고</b> ' +
            '<code>a[1]</code> → <code>(4,)</code>, <b>슬라이스는 축을 남겨</b> ' +
            '<code>a[1:2]</code> → <code>(1, 4)</code> 가 돼요. ' +
            '플레이그라운드에서 <code>0</code> 과 <code>0:1</code> 을 번갈아 눌러 확인할 수 있어요.'
        },
        {
          q: '행이 3개인 배열 <code>a</code> 에 <code>a[1:9]</code> 와 <code>a[9]</code> 를 각각 실행하면?',
          choices: [
            '둘 다 IndexError 가 나요',
            '<code>a[1:9]</code> 는 있는 만큼 2행을 주고, <code>a[9]</code> 는 IndexError 예요',
            '<code>a[1:9]</code> 는 빈 배열이고, <code>a[9]</code> 는 마지막 행을 줘요',
            '둘 다 부족한 자리를 0 으로 채워서 줘요'
          ],
          answer: 1,
          explain: '<b>슬라이스는 범위를 넘어도 에러가 아니에요</b> — 요청 범위와 실제 범위의 교집합인 1·2행을 줘요. ' +
            '반면 <b>정수 인덱싱은 값 하나를 돌려줘야 하므로</b> 없는 자리를 요구받으면 ' +
            '<code>IndexError: index 9 is out of bounds for axis 0 with size 3</code> 를 내요. ' +
            'NumPy 가 없는 값을 만들어 채우는 일은 절대 없어요.'
        },
        {
          q: '<code>a = np.arange(10)</code> 예요. 다음 중 <b>원본 <code>a</code> 가 바뀌는</b> 것은?',
          choices: [
        ...
        },
        {
          ...
          answer: 1,
          explain: '두 인덱스 배열은 <b>자리마다 짝지어</b> 좌표를 만들어요. ' +
            '<code>[0, 2]</code> 와 <code>[1, 3]</code> 에서 나오는 짝은 <code>(0,1)</code>, <code>(2,3)</code> ' +
            '두 개뿐이므로 결과는 원소 2개인 1차원 배열 <code>[1, 11]</code> 이에요. ' +
            '블록이 필요하면 <code>a[np.ix_([0,2], [1,3])]</code> 또는 ' +
            '<code>a[[0,2]][:, [1,3]]</code> 를 써요.'
        }
      ], { id: 'indexing' }));
    }
  });
