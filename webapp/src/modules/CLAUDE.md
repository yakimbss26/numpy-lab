...
예외가 하나 있다 — `quest.js` 는 장이 아니라 **과제 화면**이다. `extra: true` 로 등록하므로
학습 과정 목록·홈 타일·이전/다음 줄에 끼지 않는다. 파일명이 `ch` 로 시작하지 않는 것도 그래서다
(사전순으로 장들 뒤에 온다).
...
| `quest.js` | `quest` | **과제 화면**(장 아님). 스스로 하기 — 23문항 · 예측 · 메모 · 배지 |

가장 중요한 세 시뮬레이터: **5장 슬라이싱 플레이그라운드**, **7장 브로드캐스팅 시뮬레이터**, **8장 axis 축소기**. 학생이 가장 많이 틀리는 세 개념이다. 여기를 손볼 때는 특히 조심하라.

## 지켜야 할 것

- **말투는 해요체다**(2026-09-05 변경). 학생에게 말을 거는 글이므로 "계산해요", "바꿔 보세요" 로 쓴다.
  코드 주석은 예외로 "~한다" 체를 유지한다 — 읽는 사람이 다르다.
