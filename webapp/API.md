...
- 모든 텍스트는 **한국어**. 학생에게 말을 거는 **해요체**를 쓴다 — "계산해요", "바꿔 보세요", "괜찮아요".
  (2026-09-05 변경. 그전에는 교과서 문체 "~한다" 였다. 사용자가 학생에게 더 친근한 말투를 요청해
  11개 장 전체를 한 번에 바꿨으므로, 새 문구도 반드시 해요체로 써야 톤이 어긋나지 않는다.)
  **코드 주석과 프로젝트 문서(.md)는 그대로 "~한다" 체다** — 그쪽 독자는 학생이 아니라 개발자다.
