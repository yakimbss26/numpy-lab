...
(function () {
  ...
  Lab.register({
    ...
    sim: 'axis 축소기 · 통계 함수 탐색기',   // 시뮬레이터 목록. ' · ' 로 나눈다
...
> **`sim` 은 홈 화면에서 링크가 된다.** ` · ` 로 나눈 이름 하나하나가 누를 수 있는 칩이 되고,
> 누르면 `#/<장>?sim=<이름>` 으로 그 장을 연 다음 **그 시뮬레이터 자리까지 스크롤**한다.
> 자리는 `.card-title` · `.h-sec` · `.h-sub` · `.panel-t` 중에서 이름과 가장 가까운 것을 골라 찾는다
> (괄호 속 부연은 떼고 낱말 겹침으로 본다. 가장 긴 낱말이 없으면 남으로 친다).
>
> 그래서 **`sim` 에 적은 이름과 장 안의 카드 제목이 낱말을 하나도 공유하지 않으면 링크가 장 맨 위로만 간다.**
> 에러가 나지 않고 조용히 어긋나므로, 시뮬레이터를 추가하면 홈에서 눌러 제자리로 가는지 한 번 확인하라.
> 이름을 짧게 줄이고 싶으면 괄호를 쓰면 된다 — `axis 축소기(2·3차원)` 은 `axis 축소기` 로 찾는다.

- `render` 는 **매번 새로 호출된다**(다른 장에 갔다 오면 다시 그린다). 전역 상태를 두지 말고 `render` 안에서 지역 변수로 관리하라.
- `extra: true` 를 주면 **장이 아닌 화면**으로 등록된다(과제 화면 `quest.js` 가 그렇다).
  사이드바 "학습 과정" 목록·홈 타일·이전/다음 줄·IDLE 코드바에서 빠지고, 사이드바에는 별도
  그룹으로 붙는다. 이때 `n` 은 장 번호가 아니라 머리말에 그대로 찍히는 말("과제")이다.
...
- 모든 텍스트는 **한국어**. 교과서 문체("~한다"), 학생에게 권할 때만 "~해 보자".
  (2026-09-05 에 해요체로 바꿨다가 되돌렸다. 존댓말로 바꾸지 마라 — 이미 한 번 시도했고 사용자가
  "~하자" 쪽을 골랐다.)
...
진도는 `UI.progress` 에 `<id>:q<번호>` 키로 저장된다. 자유 입력처럼 `UI.quiz` 로 안 되는
판정을 직접 만들 때도 **같은 키 규칙**을 쓰면 사이드바 점이 그대로 동작한다
(과제 화면이 `quest:q2-1a` 식으로 그렇게 한다).
